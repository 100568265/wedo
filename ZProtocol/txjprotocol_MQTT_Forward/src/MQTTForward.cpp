...
{
    return new MQTTForward();
}

void	    MQTTForward::Init()
{
...
void	    MQTTForward::Uninit()
{


}

void	    MQTTForward::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
    readed = 0;
	if(this->GetCurPort())
	{
...
bool	    MQTTForward::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
    /*
...
bool	    MQTTForward::IsSupportEngine(ST_INT engineType)
{
    return 1;
}

bool MQTTForward::CheckTransferTableExist (int index, TransferTable*& table, List<ST_DUADDR>*& list)
{
	if (!this->GetDevice())
		return false;
...
void        MQTTForward::SendDagaMsg(char *cpayload)
{
...
void        MQTTForward::InitTransfertable()
{
...
void        MQTTForward::GetDitNameByAddr(DeviceInfo *devinfo,int id,ST_CHAR* ditName)
{
