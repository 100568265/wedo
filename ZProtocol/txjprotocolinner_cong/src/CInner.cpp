...
//            memset(ditname , 0, sizeof(ditname ) / 2);
            *(int32_t*)fullname = 0; *(int32_t*)ditname = 0;
            ...
            m_pLogger->GetInstance()->LogInfo("fullname------------------------------++++++++++%s",fullname);
            m_pLogger->GetInstance()->LogInfo("value---------------------------------++++++++++%d",vValue);
            m_pLogger->GetInstance()->LogInfo("tdd.addr------------------------------++++++++++%d",tdd.addr);
            m_pLogger->GetInstance()->LogInfo("tdd.connect---------------------------++++++++++%d",tdd.connect);
            m_pLogger->GetInstance()->LogInfo("tdd.device----------------------------++++++++++%d",tdd.device);
            m_pLogger->GetInstance()->LogInfo("tdd.type------------------------------++++++++++%d",tdd.type);
            m_pLogger->GetInstance()->LogInfo("Avalue--------------------------------++++++++++%d",GetVariableValueByName("新设备.A相电压",vValue));
            m_pLogger->GetInstance()->LogInfo("Bvalue--------------------------------++++++++++%d",GetVariableValueByName("新设备.B相电压",vValue));
            m_pLogger->GetInstance()->LogInfo("Cvalue--------------------------------++++++++++%d",GetVariableValueByName("新设备.C相电压",vValue));
