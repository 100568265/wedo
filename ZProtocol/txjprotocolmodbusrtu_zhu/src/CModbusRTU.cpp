...
void	CModbusRTU::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	...
	ST_INT nlen = this->GetCurPort()->PickBytes(pbuf, len, interval);
	if(nlen == len)
	{
...
ST_BOOLEAN	CModbusRTU::OnSend()
{
    ...
    if (!m_bInit)
    {
        this->m_pChannel->GetChannelInfo()->MaxConnects=0;
    }
	...
	{
        m_readIndex = this->m_pChannel->GetChannelInfo()->MaxConnects;
        ...
        if (this->GetDevice() == this->m_pChannel->GetDevices()->GetItem(this->GetDevices()->GetCount()-1))
        {
            if (m_readIndex < info->DataAreasCount)
                this->m_pChannel->GetChannelInfo()->MaxConnects++;
            else
                this->m_pChannel->GetChannelInfo()->MaxConnects = 0;
        }
    }
