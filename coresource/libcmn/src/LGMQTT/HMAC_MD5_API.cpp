...
typedef struct _MD5CTX {
    ST_INT32   aState[4];      // 记录数据的变化状态
    ST_INT32   aCount[2];      // 记录数据的原始长度(以bit为单位)
    ST_BYTE    aBuffer[64];    // 原始数据
} MD5CTX;

// 初始化
void MD5_Init( MD5CTX* pstruContext )
{
    const ST_INT32 *MDOriginState;
...
// MD5基本变换操作
void MD5_Transform( ST_INT32* pState, ST_INT32* px )
{
    const ST_INT32 (*MDOffTable)[16];
    ST_INT32 a,b,c,d;
...
// MD5块更新操作
void MD5_Update( MD5CTX* pstruContext, const ST_BYTE* pInput, ST_INT32 dwInputLen )
{
    ST_INT32 i, dwIndex, dwPartLen, dwBitsNum;
    ...
    if(dwInputLen >= dwPartLen)
    {
        ...
        MD5_Transform( pstruContext->aState, (ST_INT32*)pstruContext->aBuffer );

        for(i = dwPartLen; i + 63 < dwInputLen; i += 64 )
        {
            MD5_Transform( pstruContext->aState, (ST_INT32*)(pInput + i) );
        }
...
ST_INT MD5_BASE64( const ST_BYTE* inputBuffer, ST_INT inputCount, ST_CHAR* outputBuffer )
{
    ST_BYTE hash[16];
    ST_INT iByteNum;
