...
#define GETU32(plaintext) (((unsigned long)(plaintext)[0] << 24) ^ \
...
void rijndaelEncrypt(const unsigned long *rk, int nrounds, const unsigned char plaintext[16],
...
void rijndaelDecrypt(const unsigned long *rk, int nrounds, const unsigned char ciphertext[16],
...
{
    unsigned char middletext[32] = {0};
    memcpy(middletext, plaintext, sizeof(middletext));
    for (int i = 0; i < 3; ++i) {
        rijndaelEncrypt (rk, nrounds, middletext + i * 8, ciphertext + i * 8);
        memcpy(middletext + i * 8, ciphertext + i * 8, sizeof(middletext) / 2);
    }
}
...
{
    unsigned char middletext[32] = {0};
    memcpy(middletext, ciphertext, sizeof(middletext));
    for (int i = 3; i --> 0;) {
        rijndaelDecrypt (rk, nrounds, middletext + i * 8, plaintext + i * 8);
        memcpy(middletext + i * 8, plaintext + i * 8, sizeof(middletext) / 2);
    }
}
