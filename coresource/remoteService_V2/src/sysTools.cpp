        ...
        const char *targetFileNameWithPath)
{
	...
	if ((fpR = fopen(sourceFileNameWithPath, "r")) == NULL)
	{
		...
		SysLogger::GetInstance()->LogError("The file '%s' can not be opened!", sourceFileNameWithPath);
		return FAILURE;
	}
	if ((fpW = fopen(targetFileNameWithPath, "w")) == NULL)
	{
		printf("The file '%s' can not be opened! \n", targetFileNameWithPath);
		SysLogger::GetInstance()->LogError("The file '%s' can not be opened! \n", targetFileNameWithPath);
		fclose(fpR);
		return FAILURE;
	}
	...
	while ((lenR = fread(buffer, 1, BUFFER_SIZE, fpR)) > 0)
	{
		if ((lenW = fwrite(buffer, 1, lenR, fpW)) != lenR)
		{
			...
			SysLogger::GetInstance()->LogError("Write to file '%s' failed!\n", targetFileNameWithPath);
			fclose(fpR);
			fclose(fpW);
			return FAILURE;
		}
	...
	SysLogger::GetInstance()->LogInfo("copy file '%s' successful!\r\n",targetFileNameWithPath);
	return SUCCESS;
}
...
void sysTools::Getfilepath(const char *path, const char *filename,  char *filepath)
{
	...
	printf("path is = %s\n",filepath);
	SysLogger::GetInstance()->LogInfo("path is = %s\n",filepath);
}
/**
*#purpose	:删除文件夹内数据
*/
