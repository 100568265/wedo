...
void	CModbusRTU::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	...
	{
		ShowMessage ("Insufficient data length _ 1.");
		this->GetCurPort()->Clear();
	}
}
...
ST_BOOLEAN	CModbusRTU::OnSend()
{
    ...
    if (m_recharge)
    {
        //ShowMessage ("writeMeterEvents. m_recharge = 1");
        SendReadCmd(0x03, 0x3000, 0x0040);
        m_bTask = true;
        return true;
    }
	...
	if(this->HasTask() && this->GetTask(&m_curTask))
	{
		...
		else if(!strcmp(m_curTask.taskCmd, "writeMeterEvents"))		// 充值
		{
		    ShowMessage ("writeMeterEvents.");
			if (m_curTask.taskCmdCode == 0)				// 正常充值
			{
			/*	if (m_recharge)
				{
				    //ShowMessage ("writeMeterEvents. m_recharge = 1");
				...
				{*/
				    //ShowMessage ("writeMeterEvents. m_recharge = 0");
					m_count++;
					char msg[64];
					sprintf(msg,"m_curTask.taskValue : %f",m_curTask.taskValue);
					ShowMessage(msg);
					SendRecharge(tenToSixteen(m_curTask.taskValue * 100), tenToSixteen(m_count), 0x00);
					m_recharge = 1;
				//}
			}
			else if (m_curTask.taskCmdCode == 1)		// 正常退费
			{
				m_count++;
				SendRecharge(tenToSixteen(m_curTask.taskValue * 100), tenToSixteen(m_count), 0x5b);
			}
			else if (m_curTask.taskCmdCode == 2)		// 强制退费
			{
				m_count++;
				SendRecharge(tenToSixteen(m_curTask.taskValue * 100), tenToSixteen(m_count), 0x5a);
			}
		}
		return true;
	}
...
ST_BOOLEAN	CModbusRTU::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
	...
	if(m_bTask)
	{
		...
		else if(!strcmp(m_curTask.taskCmd, "writeMeterEvents"))
		{
			if (len <= 5)
			{
				ShowMessage("writer meter error");
			...
			{
			    ShowMessage("writer meter successful");
				m_curTask.taskResult.resultCode = -1;
                m_curTask.isTransfer = 1;
				Transfer(&m_curTask);
				Memset(&m_curTask,0,sizeof(m_curTask));
...
void  CModbusRTU::ProcessMemoryUInt16(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	...
	if (!strcmp(itemref.itemName, "售电次数"))
	{
		m_count = fvalue;
	}
...
unsigned long CModbusRTU::tenToSixteen(int m)
{
	unsigned long r=0,n=0;
	int i = 0;
    while(m)
    {
       n = m % 10;
...
void encryption(ST_BYTE* pBuf)
{
	/*return (WORD)((~data) ^ nRand);*/
	/*int len = 7;*/
	ST_BYTE temp = 0x00;
	ST_BYTE rTemp = 0x00;
	for(int i = 0; i < 4; i++) {
...
/*
	...
	sign		  售电标志
*/
void CModbusRTU::SendRecharge(unsigned long buyElectr, ST_UINT buyCount, ST_BYTE sign)
{
	ST_BYTE sendBuf[256];
	...
	// 表地址tr
	FillWORD(sendBuf + len, 0xAAAA);
	FillWORD(sendBuf + len + 2, 0xAAAA);
	FillWORD(sendBuf + len + 4, 0xAAAA);
	len += 6;
	...
	// 售电次数
	sendBuf[len] = buyCount & 0x00ff;
	sendBuf[len+1] = (buyCount & 0xff00) >> 8;
	len += 2;

	// 本次购电量
	FillWORD(sendBuf + len, htons(buyElectr & 0x0000ffff));
	FillWORD(sendBuf + len + 2, htons((buyElectr & 0xffff0000) >> 16));
	len += 4 + 8;

	// 透支电量
	FillWORD(sendBuf + len, 0x0000);
	// 囤积电量
	FillWORD(sendBuf + len + 2, 0x9999);
	len = len + 4 + 4;
...
/*
		...
		sendBuf[len] = (curtime.wMilliseconds+curtime.wSecond*1000)%256;	// MS   L
		sendBuf[len+1] = (curtime.wMilliseconds+curtime.wSecond*1000)/256;	// ms	h
		sendBuf[len+2] = curtime.wMinute;	// min
		sendBuf[len+3] = curtime.wHour;	// hour
		sendBuf[len+4] = curtime.wDay;	// day
		sendBuf[len+5] = curtime.wMonth;	// mon
		sendBuf[len+6] = curtime.wYear-2000;		// year*/
	len += 4 + 4;
	...
	ST_BYTE temp[2];
	*(ST_UINT16*)(temp) = get_crc16(sendBuf + 55, len - 55);

    sendBuf[len] = temp[0];
    sendBuf[len+1] = temp[1];
	len += 2;

	*(ST_UINT16*)(temp) = get_crc16(sendBuf, len);
