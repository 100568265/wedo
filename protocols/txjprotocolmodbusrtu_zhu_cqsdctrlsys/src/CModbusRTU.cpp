	...
	if(this->HasTask() && this->GetTask(&m_curTask))
	{
		...
		else if(!strcmp(m_curTask.taskCmd,"devicecontrol"))
		{
			if(m_curTask.taskCmdCode == 0)
				...
				SendYK(m_curTask.taskValue, m_curTask.taskAddr);
			else if(m_curTask.taskCmdCode == 2)
				SendReadCmd((ST_BYTE)m_curTask.taskCmdCode,m_curTask.taskValue,m_curTask.taskAddr);
			else
			{
...
void CModbusRTU::SendYK(ST_UINT writeAddr,ST_UINT count)
{
	ST_BYTE sendbuf[256];
	sendbuf[0] = (ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address;
	sendbuf[1] = 0x0F;
