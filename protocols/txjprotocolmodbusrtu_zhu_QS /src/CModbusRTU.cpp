...
void CModbusRTU::SendReadCmd(ST_BYTE code,ST_UINT readAddr,ST_UINT count)
{
	...
	*/
	ST_INT len = 0;
	ST_BYTE sendbuf[256];
    sendbuf[0] = (ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address;  //地址
    sendbuf[1] = 0x10;  //YK功能码
    //固定报文
    sendbuf[2] = 0x00;
    sendbuf[3] = 0x0C;
    sendbuf[4] = 0x00;
    ...
    //遥控信息
    sendbuf[7] = 0x00;  //补零
    sendbuf[8] = (bIsOn?0x02:0x08);
    sendbuf[9] = 0x00;  //补零
    sendbuf[10] = (bIsOn?0x02:0x08);
    len = 11;
    //校验码
    //*(ST_UINT16*)(sendbuf+len) = get_crc16(sendbuf,len);
    sendbuf[11] = get_crc16(sendbuf,len);
    sendbuf[12] = get_crc16(sendbuf,len)>>8;
...
void CModbusRTU::SendPreYK(ST_UINT writeAddr,ST_BOOLEAN bIsOn)
{
    ...
    */
    ...
    sendbuf[8] = (bIsOn?0x02:0x08);
    sendbuf[9] = 0x00;  //补零
    sendbuf[10] = (bIsOn?0x02:0x08);//(bIsOn?0x40:0x10);
    len = 11;
    //校验码
    //*(ST_UINT16*)(sendbuf+len) = get_crc16(sendbuf,len);
    sendbuf[11] = get_crc16(sendbuf,len);
