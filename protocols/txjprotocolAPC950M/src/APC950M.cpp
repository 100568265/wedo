    ...
    case FBATCH:
        {
            ...
            if(difftime(Newcurtime,oldcurtime)>3600*6) //3600*12 12hour行一次抄表 60*60*1
            {
                //oldcurtime = clock();
                time(&oldcurtime);
                SendBATCH();
...
/*            ST_UINT32 value32 = 0;
            ...
            float fv = (float)value32 * 0.1;//0.01;
/*
            int itempos = varIndex - this->GetDevice()->GetDeviceInfo()->Address;
            const ST_DataAreaItem& itemref = this->GetDevice()->GetDeviceInfo()->DataAreas[1].items[itempos-1];
...
void  APC950M::SendNDNODE(int ID)
{
    if(ID == 0){
        ...
        sendbuf[10] = '0';
        sendbuf[11] = '0';
        int idbegin = this->GetDevice()->GetDeviceInfo()->Address;
        if(idbegin == 1) idbegin = 0;
        int rid = (10*ID)+1 + idbegin;
