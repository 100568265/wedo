...
void CSY103::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	...
	if(star == len) {
		this->GetCurPort()->Clear();
		this->ShowMessage("Can not find the frame header.");
		return;
	}
		// 找到头了，进行完成 头判断
	if(frametype == 0x10)
	{
		...
		if(len < 5) {
			this->GetCurPort()->Clear();
			this->ShowMessage("Data length is not enough.");
			return;
		}
	...
	//						this->OnShowMsg("变长帧帧尾错误，系统将整个帧丢弃",0);
			...
			{
				this->GetCurPort()->Clear();
				this->ShowMessage("Garbled message");
			}
		}
		else
		{
				...
				if((m_curTask.taskCmdCode == 0) || (m_curTask.taskCmdCode == 1) || (m_curTask.taskCmdCode == 2))
				{
					if((pbuf[10] == 0xff) && (pbuf[11] == 0xff) && (m_byOnOff==pbuf[12]))
					{
					 //   if(m_curTask.taskCmdCode == 0) sleep(1500);
					    m_curTask.taskResult.resultCode = m_byOnOff;
//m_pLogger->LogDebug("m_curTask.taskResult.resultCode = m_byOnOff = %d",m_byOnOff);
...
void  CSY103::ExplainEvent(ST_BYTE* pbuf,ST_INT nlen)
{
	...
	if(stypename1 == "XSYA211")
    {
       if((nyxno>=82) && (nyxno<=97)) nIndexVarYX = 100-82+8;
    }
    else if(stypename1 == "XSYM211")
    {
        if((nyxno>=82) && (nyxno<=97)) nIndexVarYX = 100-82+24+8;
    }
    else if(stypename1 == "XSYT231")
    {
        return;
    }
    else
    ...
    if((nIndexVarYX+nyxno) >= 200)
    {
        this->UpdateValue(nIndexVarYX+nyxno,(ST_BYTE)1); //1是分，2是合
 //       m_pLogger->LogDebug("nIndexVarYX+nyxno=%d",nIndexVarYX+nyxno);
    }
 //   sprintf(name,getVarname(chSoeDevice,nIndexVarYX+nyxno));
   // if(name == "") return;
    ProtocolTask *task;
    ...
    task->ignoreBack = 1;
    task->taskTime = 1000;
//    m_pLogger->LogDebug("task->taskAddr=%d",task->taskAddr);
//    m_pLogger->LogDebug("task->taskAddr1= %d",task->taskAddr1);
    Transfer(task);
}
...
/*    ST_INT nIndexData = 12;
	...
	const ST_BYTE * dataptr = pbuf + 11;
	for (ST_INT index = 0; index < 5 * 8; ++index)
	{
		if ((index % 8) == 0)
			++dataptr;
		ST_BYTE value = (bool)(*dataptr & (0x01 << (index % 8)));
		this->UpdateValue (index + 100, value);
	}
}
...
/*	ST_INT nType = -1;
	...
	case XSYA211: {
		...
		{
			ST_FLOAT fvalue = 0.0;
			if((this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient) == 0.0)
                this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient = 1.0;
		...
		for(i = 0;i < 5; i++) //接着是5个字节的遥信
		{
			...
			for(ST_INT j = 0; j < 8; j++)
			{
				if(i==0)
				{
					if(j==2)
					{
						if(!((by << 2) & pbuf[nIndexData]))
							bJ = false;
					}
					else if(j==4)
					{
						if(!((by << 4) & pbuf[nIndexData]))
							bG = false;
					}
				}
				byValue = ((by<<j)&pbuf[nIndexData])?0x01:0x00;
			...
			if(!bJ)// && !bG)
			{
				for(ST_INT nsoe=0;nsoe<254;nsoe++)
				{
					this->UpdateValue(200+nsoe,(ST_BYTE)0);
				}
			}
			nIndexData++;
		}
	...
	// else if(stypename1 == "XSYA212")
	case XSYA212: {
		ST_INT nIndexVarYC = 0;
		ST_INT nIndexVarYX = 100;//遥信从100开始
		ST_INT nIndexData =12;//从12个字节开始
        ST_INT i =0;
		for(i = 0;i<11;i++)//有11个遥测
		{
			ST_FLOAT fvalue = 0.0;
	...
	// else if(stypename1 == "XSYA215")
	case XSYA215 : {
		ST_INT nIndexVarYC = 0;
		ST_INT nIndexVarYX = 100;//遥信从100开始
		ST_INT nIndexData =12;//从12个字节开始
        ST_INT i =0;
		for(i = 0;i<20;i++)//有20个遥测
		{
			ST_FLOAT fvalue = 0.0;
			if(this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient == 0) this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient=1;
			if(pbuf[i*2+nIndexData+1]&0x80)
			...
			for(ST_INT j=0;j<8;j++)
			{
				if(i==0)
				{
					if(j==2)
					{
						if(!((by<<2)&pbuf[nIndexData]))
						{
							this->UpdateValue(261,(ST_BYTE)0);
							this->UpdateValue(262,(ST_BYTE)0);
						}
					}
					else if(j==3)
					{
						if(!((by<<3)&pbuf[nIndexData]))
						{
							this->UpdateValue(417,(ST_BYTE)0);
							this->UpdateValue(418,(ST_BYTE)0);
							}
					}
					else if(j==6)
					{
						if(!((by<<6)&pbuf[nIndexData]))
						{
							{
								this->UpdateValue(263,(ST_BYTE)0);
								this->UpdateValue(264,(ST_BYTE)0);
							}
						}
					}
				}
	...
	// else if(stypename1 == "XSYA216")
	case XSYA216: {
		ST_INT nIndexVarYC = 0;
		ST_INT nIndexVarYX = 100;//遥信从100开始
		ST_INT nIndexData =12;//从12个字节开始
		ST_INT i =0;
		for(i = 0;i<38;i++)//有30个遥测
		{
			ST_FLOAT fvalue = 0.0;
			if(this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient == 0) this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient=1;
			if(pbuf[i*2+nIndexData+1]&0x80)
	...
	// else if(stypename1 == "XSYC211SG")
	case XSYC211SG : {
		ST_INT nIndexVarYC = 0;
		ST_INT nIndexVarYX = 100;//遥信从100开始
		ST_INT nIndexData =12;//从12个字节开始
		ST_INT i =0;
		for(i = 0;i<31;i++)//有30个遥测
		{
			ST_FLOAT fvalue = 0.0;
			if(this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient == 0) this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient=1;
			if(pbuf[i*2+nIndexData+1]&0x80)
	...
	case XSYP211T : {
		...
		this->UpdateValue(nIndexVarYX++,(ST_BYTE)(pbuf[nIndexData]-0x16)); //定值区  //1个字节
		nIndexData++;
		this->UpdateValue(nIndexVarYX++,(ST_BYTE)((pbuf[nIndexData]==0x55)?0:1)); //充电状态  //1个字节
		nIndexData++;
		nIndexData++;
		for(i=0;i<3;i++)
		{
			if(this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient == 0) this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient=1;
			ST_FLOAT dvalue = (pbuf[i*2+nIndexData+1]*256 + pbuf[i*2+nIndexData])*this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient;
	...
	// else if(stypename1 == "XSYT212Q")
	case XSYT212Q : {
		ST_INT nIndexVarYC = 0;
		ST_INT nIndexVarYX = 100;//遥信从100开始
		ST_INT nIndexData =12;//从12个字节开始
		ST_INT i =0;
		for(i = 0;i<27;i++)//有30个遥测
		{
			ST_FLOAT fvalue = 0.0;
			if(this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient == 0) this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient=1;
			if(this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient == 0) this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient=1;
...
//////////////////
	// else if(stypename1 == "XSYT221")
	case XSYT221 : {
		ST_INT nIndexVarYC = 0;
		ST_INT nIndexVarYX = 100;//遥信从100开始
		ST_INT nIndexData =12;//从12个字节开始
		ST_INT i =0;
		for(i = 0;i<24;i++)//有30个遥测
		{
			ST_FLOAT fvalue = 0.0;
			if(this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient == 0) this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient=1;
			if(pbuf[i*2+nIndexData+1]&0x80)
	...
	case XSYT231 : {
		...
		nIndexData++;
		for(ST_INT i = 0;i<4;i++) //接着是5个字节的遥信
		{
			ST_BYTE by = 0x01;
			ST_BYTE byValue = 0x00;
			for(ST_INT j=0;j<8;j++)
...
void CSY103::SendTime(void)
{
	...
	byBuf[12] = timenow->tm_sec*1000;
	byBuf[13] = (timenow->tm_sec*1000)>>8;
    byBuf[14] = (timenow->tm_min/10)*16+(timenow->tm_min%10);
	byBuf[15] = (timenow->tm_hour/10)*16+(timenow->tm_hour%10);
	byBuf[16] = (timenow->tm_mday/10)*16+(timenow->tm_mday%10);
    byBuf[17] = ((timenow->tm_mon+1)/10)*16+((timenow->tm_mon+1)%10);
    byBuf[18] = ((timenow->tm_year-100)/10)*16+((timenow->tm_year-100)%10);*/
