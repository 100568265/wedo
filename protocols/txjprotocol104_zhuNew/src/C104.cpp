...
void	   C104::Init()
{
	...
	nSendIndex = 0;
	m_bCallPH = false;
	m_readlasttime=GetTickCount();  //最后读取时间
	m_timecallall=GetTickCount();   //总召时间
	m_PHsum = 0;
	nrecnem = 0;
	m_b0701 = 0;
}
...
ST_BOOLEAN C104::OnSend()
{
	...
	{
        ShowMessage("Enter taskfunction!");
        ...
        m_b0701 = 0x00;
		unsigned long nowt = GetTickCount();
		unsigned long waittime = abs(long(nowt-m_readlasttime))/1000;
		unsigned long calltime = abs(long(nowt-m_timecallall)) /1000;
		...
		if (waittime > m_timeoutReceive)
			m_nStart = 0;
//        char temp[64];
//        sprintf(temp, "%d", calltime);
//        ShowMessage(temp);
		if (calltime > 15*60 && m_nStart)
		{
			ShowMessage("CallALL");
			this->GetCurPort()->Clear();
			STARTDT();  // 激活传输启动
...
/*			char chDeviceName[64];
			...
			///////////////////////////
			*/
			TransferEx(byValue,nIndexYX,st.Hour,st.Minute,st.Millisecond);
			this->UpdateValue(nIndexYX++,(ST_BYTE)byValue);
		}
	}
	else
...
void    C104::TransferEx (ST_BYTE statu, ST_UINT16 addr, ST_BYTE hour, ST_BYTE min, ST_UINT16 msec)
{
	struct tm t_tm;
