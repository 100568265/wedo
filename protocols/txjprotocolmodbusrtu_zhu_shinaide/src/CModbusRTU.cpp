...
void  CModbusRTU::ProcessMemorySingle(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	...
	else if(itemref.dataLen == 4)
	{
		...
		else if((itemref.codeType == 7))
        {
            ST_UINT32 value = bswap32(convert::bytes_to<uint32_t>(buf + itemref.addr, itemref.dataLen));
            if(value == 0xFFC00000)
            {
                fvalue = 0;
//                ShowMessage("this value is invalid!");
            }
            else
            {
                float tf = *((ST_FLOAT*)&value);
                fvalue = getSNDPFValue(tf);
            }
        }
	}
...
float CModbusRTU::getSNDPFValue(float regVal)
{
    float PF_Val = 0;
    if (regVal > 1)
    {
