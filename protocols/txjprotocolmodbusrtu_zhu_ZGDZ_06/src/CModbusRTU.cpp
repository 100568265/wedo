...
ST_BOOLEAN	CModbusRTU::OnSend()
{
	...
	if(this->HasTask() && this->GetTask(&m_curTask))
	{
		if(!strcmp(m_curTask.taskCmd,"singleread"))
		{
			...
			char msg[64];
			sprintf(msg,"m_curTask.taskAddr : %d",m_curTask.taskAddr);
			ShowMessage(msg);
			SendReadCmd((ST_BYTE)0x03,m_curTask.taskAddr,0x01);
			m_bTask = true;
		}
		/*
		...
		else if(!strcmp(m_curTask.taskCmd,"singlewrite"))
		{
            ...
            int ntype = 0;
            if(m_curTask.taskAddr == 0x0D00) ntype = 1;
            SendSingleWriteCmd((ST_INT)m_curTask.taskValue,m_curTask.taskAddr,ntype);
			m_bTask = true;
		}
	...
	if(info && info->DataAreasCount > 0)
	{
		...
		SendReadCmd((ST_BYTE)0x03, (ST_UINT16)info->DataAreas[m_curreadIndex].addr,
...
ST_BOOLEAN	CModbusRTU::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
	if(m_bTask)
	{
		...
		{
		    ShowMessage("receive singlewirte order");
			m_curTask.taskResult.resultCode = 0;
            m_curTask.isTransfer = 1;
            Transfer(&m_curTask);
			Memset(&m_curTask, 0, sizeof(m_curTask));
		...
		else if(!strcmp(m_curTask.taskCmd,"singleread"))
		{
			...
			m_curTask.taskResult.resultValue = (float)(pbuf[3]*256 + pbuf[4]);
			memcpy(m_curTask.taskResult.resultData,&pbuf[3],m_curTask.taskResult.resultDataLen);
			char msg[64];
			sprintf(msg,"result value :%f",m_curTask.taskResult.resultValue);
			ShowMessage(msg);
            m_curTask.isTransfer = 1;
            Transfer(&m_curTask);
			Memset(&m_curTask, 0, sizeof(m_curTask));
...
void CModbusRTU::SendYK(ST_UINT writeAddr,ST_INT data)
{
...
void  CModbusRTU::SendSingleWriteCmd(ST_INT data,ST_INT readAddr,ST_INT nType)
{
//
	ST_BYTE sendbuf[256];
	ST_INT len = 0;
	...
	if(nType == 1)      //code:10
	{
		...
		sendbuf[5] = 0x01;
		sendbuf[6] = 0x02;
		FillWORD(sendbuf + 7,data);
		len = 9;
		*(ST_UINT16*)(sendbuf + len) = get_crc16(sendbuf,len);
		this->Send(sendbuf,len+2);
	}
	else if(nType == 0)//code:06
	{
