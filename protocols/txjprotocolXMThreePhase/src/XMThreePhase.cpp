...
{
	return new XMThreePhase();
}


ST_VOID	    XMThreePhase::OnRead(ST_BYTE* pbuf, ST_INT& readed)
{
    readed = 0;
	if(this->GetCurPort())
	{
...
ST_BOOLEAN	XMThreePhase::OnSend()
{
...
ST_BOOLEAN	XMThreePhase::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
    switch(pbuf[10])
	{
	case 0x91:{ // 照明查询应答
...
ST_BOOLEAN	XMThreePhase::IsSupportEngine(ST_INT engineType)
{
    return 1;
}


void XMThreePhase::AskState()
{
...
void	XMThreePhase::Analysis21H(ST_BYTE* pbuf)
{
 //   float fvalue=0;
...
void XMThreePhase::SendYK(ST_UINT ykAddr,bool value,bool clearerror)
{
	const DeviceInfo & info = *this->GetDevice()->GetDeviceInfo();
	ST_BYTE sendbuf[256] = {0};
	sendbuf[0]	= 0x68;
	...
	sendbuf[16] = clearerror?0x01:0x00;			// 清除错误信息
	sendbuf[17] = CSCheck(sendbuf,17);
	sendbuf[18]	= 0x16;
	this->Send(sendbuf,19);
	_counter++;
}

float  XMThreePhase::bcd2Fvalue(ST_BYTE* pbuf,int len)
{
    float value=0;
    if(len == 2)
    {
