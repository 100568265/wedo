...
void	CModbusRTU::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	...
	ST_INT star = 0;
	for(; star < len - 1; ++star) {
	    // 只接收 UNB网关发给核心板的传感器透传数据
	    if (pbuf[star] == UNBAPI_VERSION && pbuf[star+1] == 0x64)
            break;
	}
	if(star > 0) {
	    ShowRecvFrame(pbuf,star);
		this->GetCurPort()->ReadBytes(pbuf, star);
		ShowMessage("Clean garbled code.");
	}
	if(star == len) {
		//全是乱码
    ...
    len = this->GetCurPort()->PickBytes(pbuf, 9, interval);
    if (len >= 9)
    {
        if (pbuf[8] != (ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address)
        {
            this->GetCurPort()->Clear();
            return;
        }
        int nlen = pbuf[2] * 256 +  pbuf[3] + 8;
        len = this->GetCurPort()->PickBytes(pbuf, nlen, interval);
        if (len >= nlen)
        {
            this->GetCurPort()->ReadBytes(pbuf, len, interval);
            readed = len;
            return;
        }
        else
        ...
        /*
        ST_BYTE fuccode = pbuf[9];
        if((fuccode & 0xF0) == 0x80) // error frame
        {
            len = 5 + 8;
        }
        ...
        else if(fuccode == 0x41)
        {
            ...
            {
                ShowMessage("No find device type.");
                this->GetCurPort()->Clear();
                return;
            }
        }
        else if(fuccode == 0x42)
        {
            if (pbuf[8] == 0x01)
                len = 8 + 11;
            else if (pbuf[8] == 0x02)
	...
	if(this->HasTask() && this->GetTask(&m_curTask))
	{
		...
		else if(!strcmp(m_curTask.taskCmd,"devicecontrol"))
		{
			if(m_curTask.taskCmdCode == 0)
				...
				SendYK(m_curTask.taskAddr,(ST_INT)m_curTask.taskValue);
...
ST_BOOLEAN	CModbusRTU::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
	...
	if(m_bTask)
	{
		...
		{
			m_curTask.taskResult.resultCode = 0;
			m_curTask.taskResult.resultDataLen = buf[2];
			memcpy(m_curTask.taskResult.resultData,&buf[3],m_curTask.taskResult.resultDataLen);
            m_curTask.isTransfer = 1;
            Transfer(&m_curTask);
			Memset(&m_curTask, 0, sizeof(m_curTask));
			return true;
	...
	else if(m_curreadIndex >= 0 && m_curreadIndex < this->GetDevice()->GetDeviceInfo()->DataAreasCount)
    {
        switch(buf[1])
        {
            ...
            {
                ST_BYTE count = buf[2];
                ProcessMemory(&buf[3], count);
            } break;
            default:
            {
                if (buf[1] == (ST_BYTE)this->GetDevice()->GetDeviceInfo()->DataAreas[m_curreadIndex].readCode)
                {
                    ST_BYTE count = buf[2];
                    ProcessMemory(&buf[3], count);
                }
...
void CModbusRTU::SendReadCmd(ST_BYTE code,ST_UINT readAddr,ST_UINT count)
{
    ...
    sendbuf[1] = code;
    sendbuf[2] = readAddr / 256;
    sendbuf[3] = readAddr % 256;
    sendbuf[4] = count / 256;
    sendbuf[5] = count % 256;
...
void CModbusRTU::SendYK(ST_UINT writeAddr,ST_INT value)
{
    ST_BYTE sendbuf[32] = {0};
    sendbuf[0] = (ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address;
    sendbuf[1] = 0x10;
    sendbuf[2] = writeAddr / 256;
    sendbuf[3] = writeAddr % 256;
    sendbuf[4] = 0x00;
    sendbuf[5] = 0x01;
    sendbuf[6] = 0x02;
    sendbuf[7] = value / 256;
...
void  CModbusRTU::SendToDev(ST_BYTE* buf,ST_INT len)
{
    ST_BYTE sendbuf[512] = {0};
    sendbuf[0] = UNBAPI_VERSION;
    sendbuf[1] = 0x65;
    sendbuf[2] = len / 256;
    sendbuf[3] = len % 256;
    sendbuf[4] = snCnt / 256;
    sendbuf[5] = snCnt % 256;
    sendbuf[6] = 0x00; // 67 yao kan chuanjinlai de
    sendbuf[7] = 0x00;
    memcpy(&sendbuf[8], buf, len);
    this->Send(sendbuf,len+8);
    snCnt ++;
}
