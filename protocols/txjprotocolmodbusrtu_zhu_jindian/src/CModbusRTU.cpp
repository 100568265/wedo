	...
	if(this->HasTask() && this->GetTask(&m_curTask))
	{
		...
		else if(!strcmp(m_curTask.taskCmd,"devicecontrol"))
		{
			...
			else if(m_curTask.taskCmdCode == 1)
            {
                ...
                SendYKAdvance(m_curTask.taskAddr,m_curTask.taskValue);
                ykAddr = m_curTask.taskAddr;
                ykIsOn = m_curTask.taskValue;
                m_bTask = true;
            }
...
void CModbusRTU::SendYKAdvance(ST_UINT writeAddr,ST_BOOLEAN bIsOn)
{
	...
	sendbuf[9] = 0x00;
	sendbuf[10] = 0x05;
	sendbuf[11] = (bIsOn ? 0xFF: 0x00);
	sendbuf[12] = 0x05;
	*(ST_UINT16*)(sendbuf + 13) = get_crc16(sendbuf,13);
