...
{
	return new TwoSpeedWind();
}


ST_VOID	    TwoSpeedWind::OnRead(ST_BYTE* pbuf, ST_INT& readed)
{
    readed = 0;
	if(this->GetCurPort())
	{
...
ST_BOOLEAN	TwoSpeedWind::OnSend()
{
...
ST_BOOLEAN	TwoSpeedWind::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
    switch(pbuf[10])
	{
	case 0xC1:{ // 普通水泵/单速风机控制器查询应答
				Analysis21H(&pbuf[13]);
			  }break;
	case 0xC0:{	// 遥控返回
...
ST_BOOLEAN	TwoSpeedWind::IsSupportEngine(ST_INT engineType)
{
    return 1;
}


void TwoSpeedWind::AskState()
{
	...
	sendbuf[9]	= 0x68;
	sendbuf[10]	= 0x41; //普通水泵/单速风机控制器状态查询
	sendbuf[11]	= 0x00;
	sendbuf[12]	= 0x00;
...
void	TwoSpeedWind::Analysis21H(ST_BYTE* pbuf)
{
 //   float fvalue=0;
    ...
    this->UpdateValue(12,bcd2Fvalue(&pbuf[15],4)); //15 16 17 18 第一通道实时合相有功功率

    this->UpdateValue(13,pbuf[19]);  //第二通道状态
    this->UpdateValue(14,bcd2Fvalue(&pbuf[20],2));
    this->UpdateValue(15,bcd2Fvalue(&pbuf[22],2));
    this->UpdateValue(16,bcd2Fvalue(&pbuf[24],2));
    this->UpdateValue(17,bcd2Fvalue(&pbuf[26],4)); //26 27 28 29 第二通道实时有功功率
}


void TwoSpeedWind::SendYK(ST_UINT ykAddr,bool value,bool clearerror)
{
	const DeviceInfo & info = *this->GetDevice()->GetDeviceInfo();
	ST_BYTE sendbuf[256] = {0};
	sendbuf[0]	= 0x68;
	...
	sendbuf[10]	= 0x40; //单相空调控制器控制   控制码：C=40H
	// 数据域长度 低前高后吧
	sendbuf[11]	= 0x03;
	sendbuf[12]	= 0x00;
	// 数据域
	sendbuf[13]	= value?0x01:0x00;
	sendbuf[14]	= value?0x01:0x00;
...
float  TwoSpeedWind::bcd2Fvalue(ST_BYTE* pbuf,int len)
{
    float value=0;
    if(len == 2)
    {
