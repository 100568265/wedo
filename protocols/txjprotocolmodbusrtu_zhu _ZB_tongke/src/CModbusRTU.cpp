	...
	if(this->HasTask() && this->GetTask(&m_curTask))
	{
		...
		else if(!strcmp(m_curTask.taskCmd,"devicecontrol"))
		{
			if(m_curTask.taskCmdCode == 0)
				...
				SendYK(m_curTask.taskAddr,(ST_UINT)m_curTask.taskValue);
...
void CModbusRTU::SendYK(ST_UINT writeAddr,ST_UINT writeValue)
{
	...
	sendbuf[5] = 0x01;
	sendbuf[6] = 0x02;
	FillWORD(sendbuf + 7,writeValue);
//	*(ST_UINT16*)(sendbuf + 9) = get_crc16(sendbuf,9);

    ST_UINT16  crc16 = get_crc16(sendbuf,9);
	//FillWORD(sendbuf1 + 13,crc16);
	memcpy(&sendbuf[9],&crc16,2);
