...
void	CModbusRTU::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	...
	interval = 100;//(interval > 2000 ? interval : 2000);
...
void CModbusRTU::ProcessMemory(ST_BYTE* buf, ST_BYTE count)
{
    if(m_curreadIndex == 4)
    {
        ShowMessage("enter SOE!");
        SpecialResolution(buf);
        return ;
    }
...
void CModbusRTU::SpecialResolution(ST_BYTE *pbuf)
{
    if(pbuf[10]>24)
    {
        int posindex = pbuf[10]-25;
        const ST_DataAreaItem& itemref = this->GetDevice()->GetDeviceInfo()->DataAreas[4].items[posindex];
        this->UpdateValue(itemref.id,1);   //SOE update
    }
    isreplay = true;
}
void  CModbusRTU::checkReset(ST_BYTE* buf)
{
    bool reset = buf[3]&0x01;
    if(reset)
    {
        ST_INT itemsize = this->GetDevice()->GetDeviceInfo()->DataAreas[m_curreadIndex].itemCount;
        for(ST_INT k = 0; k < itemsize; k++)
        {
            const ST_DataAreaItem& itemref = this->GetDevice()->GetDeviceInfo()->DataAreas[m_curreadIndex].items[k];
            this->UpdateValue(itemref.id,0);  //SOE reset
        }
    }
    char msg[64];
    sprintf(msg,"reset state :%d",reset);
    ShowMessage(msg);
}
