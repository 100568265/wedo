...
{
    //m_pLogger->LogInfo("string : %s",cpayload);
    size_t len = strlen(cpayload);
    ST_BYTE *sendbuf = new ST_BYTE[len*3];
    Memcpy(sendbuf,cpayload,len*3);
    this->Send(sendbuf,sizeof(sendbuf));
    //m_pLogger->LogInfo("byte_array : %s",sendbuf);
    delete[] sendbuf;
}
