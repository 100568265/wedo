...
void	CModbusRTU::Init()
{
	...
	m_curreadIndex = 0;
	m_readIndex = 0;
	memset(oTmArray,0,sizeof(oTmArray));
	todayNum = 0;
}
...
void	CModbusRTU::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
    ...
    if(wCRC == nCRC)
    {
        ShowMessage ("Discard this message!");
        this->GetCurPort()->ReadBytes(pbuf, star); //清除请求报文
        return;
    }
    ST_BYTE addr = pbuf[0];
    if(addr == 0x52)
...
ST_BOOLEAN	CModbusRTU::OnSend()
{
	...
	if(info && info->DataAreasCount > 0)
	{
        ...
        ShowMessage((ST_CHAR *)m_curreadIndex);
	}
	return true;
}
...
ST_BOOLEAN	CModbusRTU::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
    ...
    {
        checkFileExist();
        checkAreaEixst(this->GetDeviceId());
        delBeforFile();
...
//    checkAreaEixst(this->GetDeviceId());
	insertAllData(this->GetDeviceId(),this->GetDevice()->GetDeviceInfo()->DataAreas[m_curreadIndex].items[0]);
}
...
void  CModbusRTU::ProcessMemoryInt16(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	...
	//insertData(this->GetDeviceId(),itemref,fvalue);
//	vLists.push_back(fvalue);
    mapData[itemref.id] = fvalue;
...
void CModbusRTU::getTodayString(char *dest)
{
    struct tm *t;
    time_t tt;
    time(&tt);
    t = localtime(&tt);
    todayNum = t->tm_mday;
    sprintf(dest,"%d%02d%02d",(t->tm_year+1900),(t->tm_mon+1),(t->tm_mday));
}

void CModbusRTU::getbefordayString(char *dest)
{
    struct tm *t;
    time_t tt;
    time(&tt);
    t = localtime(&tt);
    if(t->tm_mday>7)
        sprintf(dest,"%d%02d%02d",(t->tm_year+1900),(t->tm_mon+1),(t->tm_mday-7));
    else
    {
        switch(t->tm_mon)
        {
            case 0:
            {
                sprintf(dest,"%d%02d%02d",(t->tm_year+1900),12,31-(7-t->tm_mday));
            }break;
            case 1:
            case 3:
            case 5:
            ...
            case 10:
                {
                    sprintf(dest,"%d%02d%02d",(t->tm_year+1900),(t->tm_mon),31-(7-t->tm_mday));
                }break;
            case 2:
                {
                    sprintf(dest,"%d%02d%02d",(t->tm_year+1900),(t->tm_mon),28-(7-t->tm_mday));
                }
            case 4:
            case 6:
            case 9:
            case 11:
                {
                    sprintf(dest,"%d%02d%02d",(t->tm_year+1900),(t->tm_mon),30-(7-t->tm_mday));
                }break;
            default:
                break;
        }
...
void  CModbusRTU::writeInitXml()
{
    char cToday[12] = {0},filename[64]={0};
    getTodayString(cToday);
    ...
    char tbuf[12]={0};
    getTodayString(tbuf);
    pRootElement->SetAttribute("Day",tbuf);
    pWriteDoc->LinkEndChild(pRootElement);
    pWriteDoc->SaveFile(filename);
    delete pRootElement;
}

void  CModbusRTU::checkFileExist()
{
    char cToday[12] = {0},filename[64]={0};
    getTodayString(cToday);
    sprintf(filename,"storage/%s.xml",cToday);
    ...
    if(!pReadDocument.LoadFile(filename))
    {
        ...
        ShowMessage("Open xml file error!");
    }
}

void  CModbusRTU::delBeforFile()
{
    char befday[12] = {0},filename[64]={0};
    getbefordayString(befday);
    sprintf(filename,"storage/%s.xml",befday);

    remove(filename);
}

void  CModbusRTU::checkAreaEixst(int areaID)
{
    char cToday[12] = {0},filename[64]={0};
    getTodayString(cToday);
    sprintf(filename,"storage/%s.xml",cToday);
    ...
    {
        checkFileExist();
        ShowMessage("check AreaEixst Open file failed!");
    }
  ...
  /*  {
        ...
        char abuf[8];
        sprintf(abuf,"%d",areaID);
        areaElmt->SetAttribute("deviceId",abuf);
        pRootElement->LinkEndChild(areaElmt);
        //遍历设备模板 this->GetDevice()->GetDeviceInfo()->DataAreas[m_curreadIndex].itemCount;
        int dataCount = this->GetDevice()->GetDeviceInfo()->DataAreasCount;
        for(int i =0;i<dataCount;i++)
        {
            int itemsize = this->GetDevice()->GetDeviceInfo()->DataAreas[i].itemCount;
            for(ST_INT k = 0; k < itemsize; k++)
            {
                const ST_DataAreaItem& itemref = this->GetDevice()->GetDeviceInfo()->DataAreas[m_curreadIndex].items[k];
                TiXmlElement* dataElmt = new TiXmlElement("Data");
                char buf[4];
                sprintf(buf,"%d",itemref.id);
                dataElmt->SetAttribute("id",buf);
                dataElmt->SetAttribute("name",itemref.itemName);
                areaElmt->LinkEndChild(dataElmt);
            }
        }
    ...
    {
        TiXmlElement *pRootElement = pReadDocument.RootElement();
        for (TiXmlElement* areaItem = pRootElement->FirstChildElement("Area");
            areaItem !=0; areaItem = areaItem->NextSiblingElement("Area"))
        {
            int aId = atoi(areaItem->Attribute("deviceId"));
            if(areaID == aId)
                return;//存在则直接退出
        ...
        int areaC = this->m_pChannel->GetChannelInfo()->DeviceInfos->GetCount();
        for(int i=0;i<areaC;i++)
        {
            DeviceInfo *dInfo = this->m_pChannel->GetChannelInfo()->DeviceInfos->GetItem(i);
            //不存在则创建
            TiXmlElement* areaElmt = new TiXmlElement("Area");
            char abuf[8];
            int devID = dInfo->DeviceId;
            sprintf(abuf,"%d",devID);
            areaElmt->SetAttribute("deviceId",abuf);
            pRootElement->LinkEndChild(areaElmt);

            int dataCount = dInfo->DataAreasCount;
            for(int i =0;i<dataCount;i++)
            {
                int itemsize = dInfo->DataAreas[i].itemCount;
                for(ST_INT k = 0; k < itemsize; k++)
                {
                    const ST_DataAreaItem& itemref = dInfo->DataAreas[m_curreadIndex].items[k];
                    TiXmlElement* dataElmt = new TiXmlElement("Data");
                    char buf[4];
                    sprintf(buf,"%d",itemref.id);
                    dataElmt->SetAttribute("id",buf);
...
void CModbusRTU::getDataFromID(int areaID,int dId,LISTXmlData& xdList)
{
    //////////////////////////////////////////////////////////////////////////////////////////////////////
    xdList.clear();
    char cToday[12] = {0},filename[64]={0};
    ...
    if(pReadDocument.LoadFile(filename))
    {
        ...
        for (TiXmlElement* areaItem = pRootElement->FirstChildElement("Area"); areaItem!= 0; areaItem = areaItem->NextSiblingElement("Area"))
        {
            ...
            if(areaID == aId)
            {
                for (TiXmlElement* dataItem = areaItem->FirstChildElement("Data"); dataItem!= 0; dataItem = dataItem->NextSiblingElement("Data"))
                {
                    int vId = atoi(dataItem->Attribute("id"));
                    if(dId == vId)
                    {
                        for(TiXmlElement *valueItem = dataItem->FirstChildElement("Value");valueItem!= 0;valueItem = valueItem->NextSiblingElement("Value"))
                        {
                            struct xmlDateType tmpData;
                            tmpData.areaId = areaID;
                            tmpData.id = vId;
                            string dtime(valueItem->Attribute("time"));
                            tmpData.dHour = atoi((dtime.substr(0,2)).c_str());
                            tmpData.dminute = atoi((dtime.substr(3)).c_str());
                            tmpData.fvalue = atof(valueItem->GetText());
                            xdList.push_back(tmpData);
 //                           ShowMessage("Enter value Element");
                        }
                        return ;
                    }
                }
...
void CModbusRTU::writeAllInXml(int areaID,struct curTime tData)
{
    ////////////////////////////////////////////////////////////////////////
    char cToday[12] = {0},filename[64]={0};
    getTodayString(cToday);
...
//                int dataSize = mapData.size();
                for (TiXmlElement* dataElmt = areaItem->FirstChildElement("Data"); dataElmt; dataElmt = dataElmt->NextSiblingElement("Data"))
                {
                    int dId = atoi(dataElmt->Attribute("id"));
                    map<int, float>::iterator iter;
                    iter = mapData.find(dId);
                    if(iter == mapData.end())
                    {
...
void  CModbusRTU::insertAllData(int areaID,const ST_DataAreaItem& itemref)
{
    dataList.clear();
    getDataFromID(areaID,itemref.id,dataList);
    struct tm *t;
    time_t tt;
    time(&tt);
    ...
    if(!dataList.empty())
    {
         struct xmlDateType  lastData = dataList.back();
    ...
    {
        ShowMessage("data List is empty!");
...
//(t->tm_year+1900),(t->tm_mon+1),(t->tm_mday-7)
/*
struct tm
{
  int tm_sec;
...
void  CModbusRTU::writeInXml(struct xmlDateType newData)
{
   ////////////////////////////////////////////////////////////////////////
    char cToday[12] = {0},filename[64]={0};
    getTodayString(cToday);
    ...
    else
    {
        ...
        {
            int aId = atoi(areaItem->Attribute("deviceId"));
            if(aId == newData.areaId)
            {
                for (TiXmlElement* pItem = areaItem->FirstChildElement("Data"); pItem; pItem = pItem->NextSiblingElement("Data"))
                {
                    int dId = atoi(pItem->Attribute("id"));
                    if(newData.id==dId)
                    {
                       TiXmlElement *valueElement = new TiXmlElement("Value");
                       char buffer[24];
                       sprintf(buffer,"%02d:%02d",newData.dHour,newData.dminute);//格式化时间
                       valueElement->SetAttribute("time",buffer);
                       char cValue[24];//转换后的字符串
                       sprintf(cValue, "%.2f", newData.fvalue);
                       TiXmlText *fText = new TiXmlText(cValue);
                       valueElement->LinkEndChild(fText);
                       pItem->LinkEndChild(valueElement);
                       pReadDocument->SaveFile(filename);
                       return ;//插入成功后直接退出函数
                    }
...
void  CModbusRTU::insertData(int areaID,const ST_DataAreaItem& itemref,ST_FLOAT fvalue)
{
//    if(!isClock(areaID))
//        return;
    dataList.clear();
...
void  CModbusRTU::addLabel(int areaID,const ST_DataAreaItem& itemref)
{
    ///////////////////////////////////////////////////////////////////
    char cToday[12] = {0},filename[64]={0};
    getTodayString(cToday);
    ...
    else
    {
        // 得到文件根节点
        TiXmlElement *pRootElement = pReadDocument->RootElement();
        for (TiXmlElement* areaElmt = pRootElement->FirstChildElement("Area"); areaElmt !=0; areaElmt = areaElmt->NextSiblingElement("Area"))
        {
            int aId = atoi(areaElmt->Attribute("deviceId"));
            if(aId == areaID)
            {
                TiXmlElement *DataElmt = new TiXmlElement("Data");
                char buf[4];
