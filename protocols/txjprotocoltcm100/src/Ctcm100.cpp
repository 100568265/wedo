...
ST_BOOLEAN	Ctcm100::IsSupportEngine(ST_INT engineType)
{
    return 1;
}
void	Ctcm100::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	if(this->GetCurPort())
	{
		ST_INT	len = this->GetCurPort()->PickBytes(pbuf,5,1000);
		if(len >= 5)
		{
			ST_INT star = 0;
...
ST_BOOLEAN	Ctcm100::OnSend()
{
	SendReadCmd(0x03,0x01,0x1e);
	return true;
}

ST_BOOLEAN	Ctcm100::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
	if((len-5) == (0x1e*2))
	{
		...
		for(i=0;i<2;i++)
		{
			ST_FLOAT dvalue = ((pbuf[49+i*4]*256 + pbuf[49+i*4+1])+(pbuf[49+i*4+2]*256 + pbuf[49+i*4+3])*10000)*this->GetDevice()->GetDeviceInfo()->DataAreas[0].items[nIndexVarYC].coeficient;
...
void Ctcm100::SendReadCmd(ST_BYTE code,ST_UINT16 readAddr,ST_UINT16 count)
{
	ST_BYTE sendbuf[8];
