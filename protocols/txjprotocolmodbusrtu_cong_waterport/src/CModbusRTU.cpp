...
//10功能码实现校时功能
void CModbusRTU::WriteTimeDate (ST_BYTE * pbuf, ST_INT len)
{
    //校时
    struct tm _tm;
	_tm.tm_year  = (pbuf[7]*256+pbuf[8])-1900;
	_tm.tm_mon   = (pbuf[9]*256+pbuf[10])-1;
	_tm.tm_mday  = (pbuf[11]*256+pbuf[12]);
	_tm.tm_hour  = (pbuf[13]*256+pbuf[14]);
	_tm.tm_min   = (pbuf[15]*256+pbuf[16]);
	_tm.tm_sec   = (pbuf[17]*256+pbuf[18]);
...
//实现数据补调功能
void CModbusRTU::SendHistoryData    (ST_BYTE * pbuf, ST_INT len)
{
...
void CModbusRTU::GetXMLData(int x_day,int x_hour,int x_min)
{
    ////////////////////////////////////////////////////////////////////////
...
//    getTodayString(cToday);
    ...
    sprintf(filename,"storage/%s%02s%02s.xml",t->tm_year+1900,(t->tm_mon+1),x_day);
...
//                int dataSize = mapData.size();
                ...
                {
                    for(TiXmlElement *valueItem = dataElmt->FirstChildElement("Value");valueItem!= 0;valueItem = valueItem->NextSiblingElement("Value"))
                    {
                        string dtime(valueItem->Attribute("time"));
                        int dHour = atoi((dtime.substr(0,2)).c_str());
                        int dminute = atoi((dtime.substr(3)).c_str());
                        if(x_hour == dHour && x_min == dminute)
                        {
                            float fvalue = atof(valueItem->GetText());
