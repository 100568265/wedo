...
volatile static bool debugFlag = false;
volatile static bool fcbBit = true;
volatile static bool fcvBit = false;
volatile static bool prmBit = false;
...
volatile static bool is1stData = false;
...
bool	CIEC101::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
	...
	if (!(prmBit = bool(pbuf[fc_it] & 0x40))) // 获取启动报文位, 1:启动站，报文为发送或请求报文。
		is1stData = bool(pbuf[fc_it] & 0x20);  // 获取要求访问位
	...
	case FC::CONFIRM: { // FC::RST_LINK
		...
		// 链路忙
	case FC::LINK_BUSY:
		this->OnShowMsg("Link is busy, Repeat link reset operation.", 0);
	...
	case FC::LINK_NO_WORK: {
		this->OnShowMsg("Link is not working, Reset link.", 0);
		sendqueue.push(RST_LINK);
	} break;
		// 链路还未准备好
	case FC::LINK_NO_FINISH: {
		this->OnShowMsg("Link is not finish, Repeat link reset operation.", 0);
		sendqueue.push(ASK_LINK_STATE);
	} break;
	default:
		sendqueue.push(RE_ASK_LINK_STATE);
		wDebug() << "Not Found Func Code. ";
		break;
	}
	if (is1stData) {
		is1stData = false;
		...
		case TYP::C_SC_NA_1: {//单点遥控返回
			//if ((pbuf[8] & 0x3F) == COT::ACTCON) {
				this->m_curTask.result.resultCode = 0;
				this->SendTaskResult(this->m_curTask);
		...
		default: wDebug() << "Not Found Info Object Type [" << pbuf[6] << "]";
			break;
		}
	}
...
void C101::Explain_M_SP_NA_1(ST_BYTE* pbuf,ST_INT len)  //单点信息
{
	if(pbuf[0]&0x80) //顺序
	{
...
void C101::Explain_M_SP_TA_1(ST_BYTE* pbuf,ST_INT len)  //带时标的单点信息
{
	if(pbuf[0]&0x80) //顺序
	{
		ST_INT nYXNum = pbuf[0]&0x7f;
...
void C101::Explain_M_DP_NA_1(ST_BYTE* pbuf,ST_INT len)  //双点信息
{
	if(pbuf[0]&0x80) //顺序
	{
		ST_INT nYXNum = pbuf[0]&0x7f;
...
void C101::Explain_M_DP_TA_1(ST_BYTE* pbuf,ST_INT len)  //带时标的双点信息
{
	if(pbuf[0]&0x80) //顺序
	{
		ST_INT nYXNum = pbuf[0]&0x7f;
...
void C101::Explain_M_SP_TB_1(ST_BYTE* pbuf,ST_INT len) //带CP56Time2a时标的单点信息
{
	if(pbuf[0]&0x80) //顺序
	{
		ST_INT nYXNum = pbuf[0]&0x7f;
...
void C101::Explain_M_DP_TB_1(ST_BYTE* pbuf,ST_INT len) //带CP56Time2a时标的双点信息
{
	if(pbuf[0]&0x80) //顺序
	{
		ST_INT nYXNum = pbuf[0]&0x7f;
...
void C101::Explain_M_ME_NA_1(ST_BYTE* pbuf,ST_INT len)  //测量值，规一化值
{
	if(pbuf[0]&0x80) //顺序
	{
...
void C101::Explain_M_ME_NB_1(ST_BYTE* pbuf,ST_INT len) //测量值，标度化值
{
	if(pbuf[0]&0x80) //顺序
	{
		ST_INT nYCNum = pbuf[0]&0x7f;
...
void C101::Explain_M_ME_NC_1(ST_BYTE* pbuf,ST_INT len) //测量值，短浮点数
{
	if(pbuf[0]&0x80) //顺序
	{
		ST_INT nYCNum = pbuf[0]&0x7f;
...
void C101::Explain_M_ME_ND_1(ST_BYTE* pbuf,ST_INT len) //测量值，不带品质描述词的规一化值
{
	if(pbuf[0]&0x80) //顺序
	{
		ST_INT nYCNum = pbuf[0]&0x7f;
...
void C101::Explain_M_IT_NA_1(ST_BYTE* pbuf,ST_INT len) //电能脉冲量
{
	if(pbuf[0]&0x80) //顺序
	{
		ST_INT nYCNum = pbuf[0]&0x7f;
