...
static void aes_gen_tables(void) {
	...
	for (i = 0; i < 256; i++) {
		...
		FT0[i] = (uint32) (x ^ y) ^ ((uint32) x << 8) ^ ((uint32) x << 16)
        ^ ((uint32) y << 24);
		...
		RT0[i] = ((uint32) MUL( 0x0B, y )) ^ ((uint32) MUL( 0x0D, y ) << 8)
...
void aes_set_key(aes_context *ctx, uint8 *key, int keysize) {
	...
	RK = ctx->erk;
	memset(&_key, 0, 17);
	memcpy(_key,(const char*) key,strlen((const char*)key)>16?16:strlen((const char*)key));
	//strcpy(_key, (const char*) key);
    
	for (i = 0; i < (keysize >> 5); i++) {
		GET_UINT32_BE(RK[i], _key, i << 2);
	}
	...
	switch (ctx->nr) {
        case 10:
            ...
            for (i = 0; i < 10; i++, RK += 4) {
                RK[4] = RK[0] ^ RCON[i] ^ (FSb[(uint8) (RK[3] >> 16)] << 24)
                ^ (FSb[(uint8) (RK[3] >> 8)] << 16)
                ^ (FSb[(uint8) (RK[3])] << 8)
                ^ (FSb[(uint8) (RK[3] >> 24)]);
        ...
        case 12:
            ...
            for (i = 0; i < 8; i++, RK += 6) {
                RK[6] = RK[0] ^ RCON[i] ^ (FSb[(uint8) (RK[5] >> 16)] << 24)
                ^ (FSb[(uint8) (RK[5] >> 8)] << 16)
                ^ (FSb[(uint8) (RK[5])] << 8)
                ^ (FSb[(uint8) (RK[5] >> 24)]);
        ...
        case 14:
            ...
            for (i = 0; i < 7; i++, RK += 8) {
                RK[8] = RK[0] ^ RCON[i] ^ (FSb[(uint8) (RK[7] >> 16)] << 24)
                ^ (FSb[(uint8) (RK[7] >> 8)] << 16)
                ^ (FSb[(uint8) (RK[7])] << 8)
                ^ (FSb[(uint8) (RK[7] >> 24)]);
	...
	*SK++ = *RK++;
	...
	for (i = 1; i < ctx->nr; i++) {
		...
		*SK++ = KT0[(uint8) (*RK >> 24)] ^ KT1[(uint8) (*RK >> 16)]
...
int aes_encrypt_Char(char *key, aes_text* content, aes_text* out_content) {
	int  i = 0, j = 0;
	int nLen = content->len;
	unsigned long backLen = 0;
    char* newAlock;
	aes_context ctx;
	unsigned char block[16], e_block[16];
	//char * newAlock = 0;
	//申请一个16的倍数的空间
	backLen = (nLen / 16 + 1) * 16;
    
	newAlock = (char *)malloc(backLen);
	out_content->content = (char*) malloc(backLen * sizeof(char));
	out_content->len = (int)backLen;
	//printf("aes_encrypt content:%d\n",backLen);
    if (nLen % 16 == 0){
        memset(newAlock, 16, backLen);
    } else{
        memset(newAlock, backLen - nLen, backLen);
    }

	memcpy(newAlock, content->content, nLen);
	...
	while (i < backLen) {
		...
		aes_encrypt(&ctx,  block,e_block);
		memcpy(out_content->content + j, e_block, 16);
		i += 16;
		j += 16;
	}
	free(newAlock);
...
int aes_encrypt_Hex(char *key, aes_text* content, aes_text* out_content) {
	aes_text out;
	aes_encrypt_Char(key, content, &out);
	int nLen = out.len;
	out_content->content = (char*) malloc(nLen * 2 + 1);
	//printf("aes_encrypt_Hex:%d\n",nLen);
	out_content->len = nLen * 2;
	ucharToHexLen((unsigned char*) out.content, out.len, out_content->content);
	free(out.content);
	return 0;
}
 ...
 */
void aes_decrypt(aes_context *ctx, unsigned char input[16],
                 unsigned char output[16]) {
	uint32 *RK, X0, X1, X2, X3, Y0, Y1, Y2, Y3;
...
int aes_decrypt_Char(char *key, aes_text* content, aes_text* out_content) {
	//aes_text backAlock;
	unsigned char ublock[16], e_block[16];
	int nLen = content->len;
	aes_context ctx;
	int i = 0, j = 0, n;
	out_content->content = (char *) malloc((nLen + 1) * sizeof(char));
	//生成key
	aes_set_key(&ctx, (unsigned char *) key, 128);
	memset(out_content->content, 0, (nLen + 1) * sizeof(char));
	while (i < nLen) {
		memcpy(ublock, content->content + i, 16);
		aes_decrypt(&ctx, (unsigned char *) ublock, (unsigned char *) e_block);
		memcpy(out_content->content + j, e_block, 16);
		i += 16;
		j += 16;
	}
    
	//out_content->content[j] = 0;//封闭字符串
	//ucharToStr(uch_output,output);
	//剔除用于补充16字节的字符
	//int nLen = strlen(output);
	n = nLen;
	out_content->len = nLen;
	while (n > 0) {
		if (out_content->content[n - 1] != 0) {
			out_content->len = n;
			break;
		}
		n--;
...
int aes_decrypt_Hex(char *key, aes_text* content, aes_text* out_content) {
	aes_text copy_aes_text;
	int back;
	copy_aes_text.len = content->len / 2;
    
	copy_aes_text.content = (char *) malloc(copy_aes_text.len * sizeof(char));
	memset(copy_aes_text.content, 0, copy_aes_text.len * sizeof(char));
	hexToUChar(content->content, (unsigned char *) copy_aes_text.content);
	back = aes_decrypt_Char(key, &copy_aes_text, out_content);
	free(copy_aes_text.content);
	return back;
}
/*
 * AES-CBC buffer encryption
 */
void aes_cbc_encrypt(aes_context *ctx, unsigned char iv[16],
 ...
 */
void aes_cbc_decrypt(aes_context *ctx, unsigned char iv[16],
                     unsigned char *input, unsigned char *output, int len) {
	int i;
...
int main(int argc, char *argv[])
{
	...
	/*aes_context ctx;
	 ...
	 aes_encrypt(&ctx,(unsigned char *)mingwen,key);
	 char miwen111_hex[1024];
	 ucharToHex(key,miwen111_hex);*/
	aes_text aess;
	aess.content = mingwen;
	aess.len = strlen(mingwen);
	aes_text out;
	printf("%s\n",mingwen);
	int cc = aes_encrypt_Hex((char *)key,&aess,&out);
	printf("%s\n",out.content);
    
	aes_text out1;
	int dd = aes_decrypt_Hex((char *)key,&out,&out1);
	//Paes_text dd = &aes_decrypt_Hex("1234567890123456",*cc);
	printf("%s\n",out1.content);
