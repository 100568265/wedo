...
ST_BOOLEAN	CModbusRTU::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
	...
	{
		if (this->GetDevice()->GetDeviceInfo()->DataAreas[m_curreadIndex].addr == 0x3600)
		{
			EventTransHelper (pbuf + 3, pbuf[2]);
		}
		else {
			ProcessMemory(pbuf + 3, pbuf[2]);
		}
	}
	return true;
}
...
struct event_type
{
	...
	inline uint16_t addr () const { return addrh * 256 + addrl; }
	inline uint16_t year () const { return yearh * 256 + yearl + 2000; }
	inline uint16_t msec () const { return msech * 256 + msecl; }
};

#pragma pack(pop)		// 恢复默认内存对齐

int  CModbusRTU::EventTransHelper (ST_BYTE* buf, ST_BYTE len)
{
	event_type * ev = (event_type*)buf;
	if (ev->meaning)
	{
		const ST_DeviceDataArea & dataarea = this->GetDevice()->GetDeviceInfo()->DataAreas[m_curreadIndex];
		const ST_DataAreaItem * itemptr = 0;
		int itemsize = dataarea.itemCount;
		for (int i = 0; i < itemsize; ++i)
		{
			if (dataarea.items[i].dataLen == ev->meaning)
				itemptr = dataarea.items + i;
		}
		if (!itemptr) {
			this->ShowMessage("[Event Record] Not Found Dit.");
			return -1;
		}

        this->UpdateValue(itemptr->id, (ST_BYTE)(!!ev->corrval));
		...
		task.taskAddr       = itemptr->id;
		task.taskValue      = (bool)ev->corrval;
		task.taskAddr1      = task.deviceId;
		task.ignoreBack     = 1;
		task.taskTime       = 2000;
		task.taskParam[0]   = (ev->year()) % 256;
		task.taskParam[1]   = (ev->year()) / 256;
		task.taskParam[2]   =  ev->mon;
		task.taskParam[3]   =  ev->day;
		task.taskParam[4]   =  ev->hour;
		task.taskParam[5]   =  ev->min;
		task.taskParam[6]   =  ev->msec() / 1000;
		task.taskParam[7]   = (ev->msec() % 1000) % 256;
		task.taskParam[8]   = (ev->msec() % 1000) / 256;
