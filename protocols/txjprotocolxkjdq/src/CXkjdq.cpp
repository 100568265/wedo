...
ST_BOOLEAN	CXkjdq::IsSupportEngine(ST_INT engineType)
{
    return 1;
}
...
void	CXkjdq::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	readed = 0;
	if(! this->GetCurPort())
		return;
...
ST_BOOLEAN	CXkjdq::OnSend()
{
	...
	if(this->HasTask() && this->GetTask(&m_curTask))
	{
        if(!strcmp(m_curTask.taskCmd,"devicecontrol"))
		{
            ...
            m_byOnOff = (ST_BYTE)m_curTask.taskValue?0x12:0x11;
	...
	else
	{
		...
		ST_BYTE sendbuf[10];
		sendbuf[0]=0x55;
		sendbuf[1]=(ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address;
		sendbuf[2]=0x10;
		sendbuf[3]=0x00;
		sendbuf[4]=0x00;
		sendbuf[5]=0x00;
...
ST_BOOLEAN	CXkjdq::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
	ST_INT pos = 0;
	if (pbuf[2]==0x10)
	{
		...
		//	byvalue = pbuf[4+i]&(by<<j);
			byvalue =(pbuf[4]>>j)&by;
			this->UpdateValue(j,(ST_BYTE)byvalue?1:0);
		}
		...
		//	byvalue = pbuf[3+i]&(by<<k);
			byvalue =(pbuf[3]>>k)&by;
			this->UpdateValue(8+k,(ST_BYTE)byvalue?1:0);
		}
	...
	{
        m_curTask.taskResult.resultCode = 0;  //判断到有回应报文则返回任务成功
		m_curTask.isTransfer = 1;
        Transfer(&m_curTask);  //将resultcode置为0 再转发则时返回任务成功
...
ST_BOOLEAN   CXkjdq::YKExecut(ST_INT nroute,ST_BYTE byOnOff)
{
    ST_BYTE sendbuf[10];
	sendbuf[0]=0x55;
	sendbuf[1]=0x01;
