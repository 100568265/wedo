...
void CModbusRTU::getDataFromID(int areaID,int dId,LISTXmlData& xdList)
{
 ...
 //               ShowMessage("Not found Data ID");
            }
        }
//        ShowMessage("Not found area ID");
...
*/
bool  CModbusRTU::isClock(int areaID)
{
    struct cpTimer tmper = oTmArray[areaID];
    struct tm *t;
    time_t tt;
    time(&tt);
    t = localtime(&tt);   //Get time message
    if(tmper.c_mon>(t->tm_mon+1))
        return true;
    if(tmper.c_day>(t->tm_mday))
        return true;
    int curMin = (t->tm_hour*60)+t->tm_min;
    oTmArray[areaID].c_mon = t->tm_mon+1;
    oTmArray[areaID].c_day = t->tm_mday;
    oTmArray[areaID].c_mon = curMin;
    if((curMin-(tmper.c_min))>=5)
        return true;
    else
    {
        ShowMessage("time has not arrived");
        return false;
    }
