...
void  CModbusRTU::ProcessMemoryInt16(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	...
	{
		if((itemref.codeType == 2))  // bcd back
		{
			ST_UINT16 value = 0;
			memcpy (&value, buf + itemref.addr, sizeof(value));
		...
		else if (itemref.codeType == 5) // bcd front
		{
		    ST_BYTE *pbuf = buf + itemref.addr;
		    fvalue = ((pbuf[0] & 0x70) >> 4)*10 + (pbuf[0] & 0x0f);
		    fvalue = fvalue * 100;
		    fvalue += ((pbuf[1] & 0xF0) >> 4)*10 + (pbuf[1] & 0x0f);
		}
	}
	else if(itemref.dataLen == 3)
	{
        if((itemref.codeType == 5)) // bcd front
		{
		    ST_UINT32 value = 0;
			ST_BYTE *pbuf = (buf + itemref.addr);
			value = ((pbuf[0] & 0x70) >> 4)*10 + (pbuf[0] & 0x0F);
            value = value * 100;
			value += ((pbuf[1] & 0xf0) >> 4)*10 + (pbuf[1] & 0x0F);
			value = value * 100;
			value += ((pbuf[2] & 0xf0) >> 4)*10 + (pbuf[2] & 0x0F);
			fvalue = (ST_FLOAT)value;

			if (pbuf[0] & 0x80)
                //fvalue = -1*(fvalue - 0xFFFFFF);
                fvalue = -1 * fvalue;
		}
		else if(itemref.codeType == 2)
		{
		    ST_UINT32 value = 0;
			ST_BYTE *pbuf = (buf + itemref.addr);
			value = ((pbuf[2] & 0x70) >> 4)*10 + (pbuf[2] & 0x0F);
            value = value * 100;
			value += ((pbuf[1] & 0xf0) >> 4)*10 + (pbuf[1] & 0x0F);
			value = value * 100;
			value += ((pbuf[0] & 0xf0) >> 4)*10 + (pbuf[0] & 0x0F);
			fvalue = (ST_FLOAT)value;

			if (pbuf[0] & 0x80)
                //fvalue = -1*(fvalue - 0xFFFFFF); // ?三字节反码
                fvalue = -1 * fvalue;
		}
	}
...
void  CModbusRTU::ProcessMemoryInt32(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	...
	else if(itemref.dataLen == 2)
	{
		...
		{
		    ST_BYTE *pbuf = buf + itemref.addr;
		    fvalue = ((pbuf[0] & 0xF0) >> 4)*10 + (pbuf[0] & 0x0f);
		    fvalue = fvalue * 100;
		    fvalue += ((pbuf[1] & 0xF0) >> 4)*10 + (pbuf[1] & 0x0f);
		}
	...
	else if(itemref.dataLen == 4)
	{
		...
		else if((itemref.codeType == 5))
		{
			...
			*/
            ...
            value = value * 100;
            value += ((pbuf[3] & 0xF0)>>4)*10 + (pbuf[3]&0x0F);
...
void CModbusRTU::SendYK(ST_UINT writeAddr,ST_BOOLEAN bIsOn)
{
	...
	sendbuf[1] = 0x10;
	sendbuf[2] = 0xC0;
    //sendbuf[3] = bIsOn ? 0x3D:0x3C;
    sendbuf[3] = bIsOn ? 0x3D:0x3A;
    sendbuf[4] = 0x00;
	sendbuf[5] = 0x01;
	*(ST_UINT16*)(sendbuf + 6) = get_crc16(sendbuf,6);
	this->Send(sendbuf,8);
