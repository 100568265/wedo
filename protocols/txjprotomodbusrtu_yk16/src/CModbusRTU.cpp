...
void CModbusRTU::SendYK(ST_UINT writeAddr,float data)
{
	ST_BYTE sendbuf[64] = {0};
	sendbuf[0] = (ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address;
	sendbuf[1] = 0x10;
	...
	uint16_t dat = data;
	FillWORD(sendbuf + 7, dat);

	uint16_t crc = get_crc16(sendbuf, 9);
	memcpy(sendbuf + 9, &crc, sizeof(crc));
