...
void	CInner::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
    if(this->HasTask() && this->GetTask(&m_curTask))
    {
        ...
        {
            char msg1[128];
            sprintf(msg1,"write meter .taskResult.resultCode : %d",m_curTask.taskResult.resultCode);
            ShowMessage(msg1);
            SendWMEcho(m_curTask);
        }
...
ST_BOOLEAN	CInner::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
    ...
    switch(pbuf[2]) {
        ...
        case 0x10: {
            switch (pbuf[7]) {
                ...
                case 0x02: { // YT
                    ...
                    task.taskCmdCode = pbuf[8];
                    task.taskAddr    = pbuf[9] + pbuf[10] * 256;
                    task.taskAddr1    = pbuf[11] + pbuf[12] * 256;
                    float value = 0;
                    memcpy(&value, pbuf + 13, sizeof(value));
        ...
        case 0x06:{
            this->ShowMessage("recive writeMeterEvents order");
            ProtocolTask task;
            task.isTransfer     = true;
            task.transChannelId = -1;
            task.transDeviceId  = pbuf[5] + pbuf[6] * 256;
            Strcpy(task.taskCmd, "writeMeterEvents");
            task.taskCmdCode = pbuf[7];
            task.taskValue = 0; //
            //float value; //= *(unsigned long *)()
            //ST_UINT32 u32Value = pbuf[8]+pbuf[9]*256+pbuf[10]*256*256+pbuf[11]*256*256*256;
            float *wmValue = (float *)&pbuf[8];
            task.taskValue   = *wmValue;
            char msg[128];
            sprintf(msg,"task.taskValue : %f",task.taskValue);
            this->ShowMessage(msg);
            Transfer(&task);
        }break;
        case 0x05:{
            this->ShowMessage("recive read fixValue order");
            ProtocolTask task;
            task.isTransfer     = true;
            task.transChannelId = -1;
            task.transDeviceId  = pbuf[5] + pbuf[6] * 256;
            Strcpy(task.taskCmd, "singleread");
            task.taskCmdCode = pbuf[7];
            task.taskAddr = pbuf[8] + pbuf[9] * 256;
            task.taskAddr1 = pbuf[10] + pbuf[11] * 256;
            Transfer(&task);
        }
        case 0x20: {
...
void   CInner::SendWMEcho(ProtocolTask& curTask)
{
    this->ShowMessage("return write meter result!");
    ST_BYTE bySendbuffer[64];
    bySendbuffer[0] = 0xaa;
    bySendbuffer[1] = 0xbb;
    bySendbuffer[2] = 0x86;
    ...
    bySendbuffer[6] = (curTask.transDeviceId&0xff00)>>8;
    bySendbuffer[7] = curTask.taskResult.resultCode;
    //m_pLogger->LogDebug("write meter .taskResult.resultCode : %d",curTask.taskResult.resultCode);
    this->Send(&bySendbuffer[0],8);
    Memset(&curTask, 0, sizeof(curTask));
}

void    CInner::SendRFEcho(ProtocolTask& curTask) //Read fix value
{
    this->ShowMessage("return Read FixValue result!");
    ST_BYTE bySendbuffer[64];
    bySendbuffer[0] = 0xaa;
    bySendbuffer[1] = 0xbb;
    bySendbuffer[2] = 0x85;
    bySendbuffer[3] = 0x07 + curTask.taskResult.resultDataLen;
    bySendbuffer[4] = 0x00;
    bySendbuffer[5] = curTask.transDeviceId&0xff;   //deviceid
    bySendbuffer[6] = (curTask.transDeviceId&0xff00)>>8;
    float fvalue = curTask.taskResult.resultValue;
    memcpy(&bySendbuffer[7],&fvalue,sizeof(fvalue)); // 7 8 9 10  resultValue
    bySendbuffer[11] = curTask.taskResult.resultDataLen;
    memcpy(&bySendbuffer[12],curTask.taskResult.resultData,curTask.taskResult.resultDataLen); //resultData
    this->Send(&bySendbuffer[0],12 + curTask.taskResult.resultDataLen);
    Memset(&curTask, 0, sizeof(curTask));
}
...
void CInner::SendYTEcho (ProtocolTask & _task)
{
    ...
    data[7] = 0x00;//_task.taskResult.resultCode;
    char msg[64];
    sprintf(msg,"single write result code : %d",_task.taskResult.resultCode);
    ShowMessage(msg);
    this->Send(&data[0], 8);
