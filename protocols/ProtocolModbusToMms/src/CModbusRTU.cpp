...
CModbusRTUToMMS::CModbusRTUToMMS()
{
	m_curreadIndex=0;
	m_readIndex=0;
	m_bTask=false;
	m_MMS = new MMSServer();
	printf("new mms object. \n");
}
...
{
    return new CModbusRTUToMMS();
}

bool	CModbusRTUToMMS::IsSupportEngine(ST_INT engineType)
{
    return 1;
}

void	CModbusRTUToMMS::Init()
{
	printf("call Init. \n");
	m_bTask = false;
	m_curreadIndex = 0;
	m_readIndex = 0;
	m_MMS->Init();
	m_thread.Start(MMSProc,this,false);
}

void	CModbusRTUToMMS::Uninit()
{
	m_MMS->Stop();
}

ST_VOID	*CModbusRTUToMMS::MMSProc(ST_VOID *param)
{
    CModbusRTUToMMS* obj=(CModbusRTUToMMS*)param;
    obj->m_MMS->Start();
    return 0;
}

void	CModbusRTUToMMS::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	readed = 0;
	if(! this->GetCurPort())
		return;
...
bool	CModbusRTUToMMS::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
	if(m_bTask)
	{
...
void CModbusRTUToMMS::ProcessMemory(ST_BYTE* buf, ST_BYTE count)
{
...
void  CModbusRTUToMMS::ProcessMemorySByte(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	...
	else if(itemref.dataLen == 2)
	{
		...
		float fvalue = value;
		if(itemref.coeficient != 0)
			fvalue *= itemref.coeficient;

		this->UpdateValue(itemref.id, fvalue);
	}
}

void  CModbusRTUToMMS::ProcessMemoryInt16(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	ST_FLOAT fvalue = 0;
	if(itemref.dataLen == 1)
	{
	...
	this->UpdateValue(itemref.id, fvalue);
	this->Map_YcChge(itemref.id, fvalue);//Int16
	//this->UpdateValue(k,fvalue);
}

void  CModbusRTUToMMS::ProcessMemoryInt32(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	ST_DOUBLE fvalue = 0;
	if(itemref.dataLen == 1)
	{
...
void  CModbusRTUToMMS::ProcessMemoryByte(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	ProcessMemorySByte(buf, itemref);
}

void  CModbusRTUToMMS::ProcessMemoryUInt16(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	ST_FLOAT fvalue = 0;
	if(itemref.dataLen == 1)
	{
	...
	this->UpdateValue(itemref.id, fvalue);
//	this->Map_YcChge(itemref.id, fvalue);
	//this->UpdateValue(k,fvalue);
}

void  CModbusRTUToMMS::ProcessMemoryUInt32(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	ST_DOUBLE fvalue = 0;
	if(itemref.dataLen == 1)
	{
...
void  CModbusRTUToMMS::ProcessMemorySingle(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	ST_FLOAT fvalue = 0;
	if(itemref.dataLen == 1)
	{
...
void  CModbusRTUToMMS::ProcessMemoryBoolean(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	ST_BYTE value = (ST_BOOLEAN)(*(buf + itemref.addr));
	this->UpdateValue(itemref.id, value);
}

void  CModbusRTUToMMS::ProcessMemoryString(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{

}

void  CModbusRTUToMMS::ProcessMemoryBytes(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{

}

void  CModbusRTUToMMS::ProcessMemoryDouble(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	ST_DOUBLE fvalue = 0;
	if(itemref.dataLen == 1)
	{
...
void  CModbusRTUToMMS::ProcessMemoryDecimal(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{

}

void  CModbusRTUToMMS::ProcessMemoryDateTime(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
...
void  CModbusRTUToMMS::ProcessMemoryInt64(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
...
void  CModbusRTUToMMS::ProcessMemoryUInt64(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
...
void CModbusRTUToMMS::SendReadCmd(ST_BYTE code,ST_UINT readAddr,ST_UINT count)
{
	ST_BYTE sendbuf[8];
...
void CModbusRTUToMMS::SendYK(ST_UINT writeAddr,ST_BOOLEAN bIsOn)
{
...
void CModbusRTUToMMS::SendPreYK(ST_UINT writeAddr,ST_BOOLEAN bIsOn)
{
	ST_BYTE sendbuf[256];
	sendbuf[0] = (ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address;
	sendbuf[1] = 0x05;
...
void CModbusRTUToMMS::SendYT(ST_UINT writeAddr,ST_UINT wIndex,ST_BOOLEAN bIsOn)
{
	ST_BYTE sendbuf[256];
	sendbuf[0] = (ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address;
	sendbuf[1] = 0x05;
...
void  CModbusRTUToMMS::SendSingleWriteCmd(ST_FLOAT data,ST_INT readAddr,ST_INT nType)
{
//	this->OnShowMsg("-----------------------------------------设置单点值",0);
	...
	if(nType == 0) //两个字节WORD
	{
		...
		FillWORD(sendbuf + 2,readAddr);
		uint16_t value = data;
		FillWORD(sendbuf + 4,value);
		len = 6;
		value = get_crc16(sendbuf, len);
		memcpy(sendbuf + len, &value, sizeof (value));
...
void  CModbusRTUToMMS::SendWriteCmd(ST_UCHAR* pData,ST_UINT dataLen,ST_UINT addr)
{
...
void CModbusRTUToMMS::FillWORD(ST_BYTE* buf,ST_UINT v)
{
...
ST_UINT CModbusRTUToMMS::GetAppWORDValue(ST_UINT v,ST_INT codetype)
{
...
ST_UINT32 CModbusRTUToMMS::htonl1(ST_UINT32 dv)
{
...
void  CModbusRTUToMMS::SendWriteCmd(ST_UCHAR* pData,ST_UINT dataLen)
{
...
void CModbusRTUToMMS::Map_YcChge(int index,float value )
{ // 发送变化遥测数据
		int ioAddr = index;
		printf("index=%d,value=%f\n",index,value);
		ST_YCDATA_MAP* yc_map = m_MMS->YCMapArray->YCMapInst[0].YcMap[ioAddr];
		if(yc_map==NULL){
			return;
		}
		DATA_MAP* map = yc_map->stVal.valMap;
		if (map == NULL){
			return;
		}
		float *data = (float*) map->dataPtr;
		*data = value;
}
