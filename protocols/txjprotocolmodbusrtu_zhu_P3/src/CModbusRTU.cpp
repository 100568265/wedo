...
ST_BOOLEAN	CModbusRTU::OnSend()
{
	...
	if(this->HasTask() && this->GetTask(&m_curTask))
	{
		...
		{
            ST_UINT taskWriteAddr = (m_curTask.taskAddr1 << 8) & 0xff00;
			taskWriteAddr += (m_curTask.taskAddr & 0x00ff);

			if(m_curTask.taskCmdCode == 0)
				SendPreYK(taskWriteAddr,m_curTask.taskValue);
			else if(m_curTask.taskCmdCode == 1)
				SendYK(taskWriteAddr,m_curTask.taskValue);
