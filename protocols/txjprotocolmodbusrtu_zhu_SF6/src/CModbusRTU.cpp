...
void	CModbusRTU::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	...
	ST_INT len = this->GetCurPort()->PickBytes(pbuf, 6, interval);
	if(len < 6) {
	...
	{
		//读数据返回
		ST_BYTE readCount = pbuf[2 + star]*256+pbuf[3 + star];
		len = readCount*2 + 6;
	}
...
ST_BOOLEAN	CModbusRTU::OnProcess(ST_BYTE* pbuf,ST_INT len)
{
	...
	{
		ST_BYTE count = pbuf[2]*256+pbuf[3];
		ProcessMemory(&pbuf[4], count*2);
	}
	return true;
}
