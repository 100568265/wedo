...
void	CModbusRTU::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	...
	{
	    len = this->GetCurPort()->PickBytes(pbuf, 8, 1000);
	    if (len == 8)
            len = 9 + pbuf[6];
	}
...
void CModbusRTU::SendPreYK(ST_UINT writeAddr,ST_BOOLEAN bIsOn)
{
	...
	sendbuf[4] = 0x55;
	sendbuf[5] = 0xFF;
	ST_UINT16 crc = get_crc16(sendbuf,6);
    *(ST_UINT16*)(sendbuf + 6) = get_crc16(sendbuf,6);
	this->Send(sendbuf,8);
 //   YKIndex = 1;
    /*
...
void CModbusRTU::SendYT(ST_UINT writeAddr,ST_UINT wIndex,ST_BOOLEAN bIsOn)
{
	...
	sendbuf[0] = (ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address;
	sendbuf[1] = 0x05;
	sendbuf[2] = (ST_BYTE)writeAddr / 256;
	sendbuf[3] = (ST_BYTE)writeAddr % 256;
...
//	this->OnShowMsg("-----------------------------------------设置单点值",0);
	...
	if(nType == 0) //两个字节WORD
	{
		sendbuf[0] = (ST_BYTE)this->GetDevice()->GetDeviceInfo()->Address;
		sendbuf[1] = 0x06;
		sendbuf[2] = (ST_BYTE)readAddr / 256;
		sendbuf[3] = (ST_BYTE)readAddr % 256;
		ST_UINT16 value = data;
		sendbuf[4] = (ST_BYTE)value/ 256;
		sendbuf[5] = (ST_BYTE)value % 256;
		value = get_crc16(sendbuf, 6);
		sendbuf[6] = (ST_BYTE)value / 256;
		sendbuf[7] = (ST_BYTE)value % 256;
		this->Send(sendbuf,8);
	}
