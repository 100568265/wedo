...
//	if (!this->GetDevice())
 ...
 /*
	...
	*/
	const DeviceInfo & info = *this->GetDevice()->GetDeviceInfo();
	int nAddr = (info.Addressex[ 0]-'0')*1000+(info.Addressex[1]-'0')*100+(info.Addressex[2]-'0')*10
                +(info.Addressex[ 3]-'0');
    unsigned char lineAddr[2] = {0};
    DectoHex(nAddr,lineAddr,2) ;  //十进制转16进制
	m_obj->option.laddr = lineAddr[1];
	m_obj->option.laddr1 = lineAddr[0];
	m_obj->option.paddr = lineAddr[1];
	m_obj->option.paddr1 = lineAddr[0];
	// this->GetDevice()->GetDeviceInfo()->AddressEx;
    Newcurtime=clock();
	sendserchtime=clock();
}
//远方位置
int distancePos[8] = {0,9,21,32,42,52,62};
void CIEC101::Uninit()
{
}
...
bool CIEC101::OnSend ()
{
	...
	/*
    //CLOCKS_PER_SEC
	if ((Newcurtime-sendserchtime) ) //大于1800秒就是30分钟
   {
 //   Thread::SLEEP (3000);
    ShowMessage("Enter func");
    sendserchtime=clock();
//        ProtocolTask curTask;
    struct tm t_tm;
	...
	task.deviceId       = 1;//this->GetDevice()->GetDeviceInfo()->DeviceId;
	task.taskCmdCode    = 0;
	task.taskParamLen   = 14;
	task.taskAddr       = 1;//addr;
	task.taskValue      = 1;//statu;
	task.taskAddr1      = 1;//this->GetDevice()->GetDeviceInfo()->DeviceId;
	task.ignoreBack     = 1;
	task.taskTime       = 1000;
	...
	task.taskParam[4]   =  5;
	task.taskParam[5]   =  20;//min;
	task.taskParam[6]   =  6000 / 1000;
	task.taskParam[7]   = (6000 % 1000) % 256;
	task.taskParam[8]   = (6000 % 1000) / 256;
	task.taskParam[9]   = 1;//statu;
	task.taskParam[10]  = 2 % 256;
	task.taskParam[11]  = 2 / 256;
...
//			papdu->nsq[msgcount].ioa.l16 = (iter + 0x4001) % 65536;
            ...
            if(fabs( (m_fdatavalue[nIndex] - stvar.fVal)/m_fdatavalue[nIndex]) > 0.2)
            {
                m_hv.add (iter + 0x4001, stvar);
...
void CIEC101::TaskHandlerToLower (const uint8_t* data)
{
    ...
    int devAddr = data[13];//获取设备号 distancePos[devAddr]
    ShowMessage("devAddr");
    ST_DUADDR  *duaddr = tablelist->GetItem(distancePos[devAddr]);  //从设备号获取就地数值
    TRVariable *TRVar  = trantable->GetTRVariable(duaddr);
    ST_VARIANT stvar;
    ST_INT nR = GetVariableValueByAddr(*duaddr, stvar);
    if (nR < 0) stvar.ulVal = 0;
