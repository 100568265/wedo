...
void CGPS::OnRead(ST_BYTE* pbuf, ST_INT& readed)
{
    ...
    ST_INT len = this->GetCurPort()->PickBytes(pbuf, 24, 5000);
    if (len < 20) {
    ...
    if (pbuf[0] == 0x01) {
        if (pbuf[22] == '\r' && pbuf[23] == '\n') {
            readed = this->GetCurPort()->ReadBytes(pbuf, 24);
            return;
        }
        len = this->GetCurPort()->PickBytes(pbuf, 26, 100);
        if (len < 26) {
            this->ShowMessage ("Insufficient data length.");
            this->GetCurPort()->Clear();
            return;
        }
        if (pbuf[24] == '\r' && pbuf[25] == '\n') {
            ST_UINT16 crc = get_crc16 (pbuf, 22);
            if (memcmp (pbuf + 22, &crc, sizeof(crc))) {
                this->ShowMessage ("Check error.");
                this->GetCurPort()->Clear();
            }
            else {
                readed = this->GetCurPort()->ReadBytes(pbuf, 26);
            }
            return;
        }
...
ST_BOOLEAN CGPS::OnProcess(ST_BYTE* pbuf, ST_INT len)
{
	...
	if(isdigit(pbuf[1] ) && isdigit(pbuf[2] )
    && isdigit(pbuf[3] ) && isdigit(pbuf[4] )
    && isdigit(pbuf[6] ) && isdigit(pbuf[7] )
    && isdigit(pbuf[9] ) && isdigit(pbuf[10])
    && isdigit(pbuf[12]) && isdigit(pbuf[13])
    && isdigit(pbuf[15]) && isdigit(pbuf[16])
    && isdigit(pbuf[18]) && isdigit(pbuf[19]))
    {
        struct tm  t_tm;
        memset (&t_tm, 0, sizeof(t_tm));

        t_tm.tm_hour = (pbuf[12]-'0') * 10 + (pbuf[13]-'0');
        t_tm.tm_min  = (pbuf[15]-'0') * 10 + (pbuf[16]-'0');
        t_tm.tm_sec  = (pbuf[18]-'0') * 10 + (pbuf[19]-'0');
        t_tm.tm_year = (pbuf[2] -'0') * 100+ (pbuf[3] -'0') * 10 + (pbuf[4]-'0') + 100;
        t_tm.tm_mon  = (pbuf[6] -'0') * 10 + (pbuf[7] -'0') - 1;
        t_tm.tm_mday = (pbuf[9] -'0') * 10 + (pbuf[10]-'0');
