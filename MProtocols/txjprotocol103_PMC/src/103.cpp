...
ST_BOOLEAN	C103::OnSend()
{
	...
	else if (SendState==0x03)//召唤二级数据
	{
		...
		if (abs(Newcurtime-OneTime)>5 * CLOCKS_PER_SEC)//读取一次数据
		{
			OneTime=Newcurtime;
			//SendASDU21(0x01,0x01);  //读取测量数据
			ShowMessage("read 1");
			SendASDU21(0xf1,0x100);

			return true;
		}

/*
		if (abs(Newcurtime-TwoTime)>5 * CLOCKS_PER_SEC)//读取二次数据
		{
			TwoTime=Newcurtime;
			ShowMessage("read 2");
//			SendASDU21(0x02,0x02);   //读取状态信息
			SendASDU21(0xf4,0x04);
			return true;
		}
		if (abs(Newcurtime-TwoTime)>6 * CLOCKS_PER_SEC)//读取二次数据
		{
			TwoTime=Newcurtime;
			ShowMessage("read 2");
//			SendASDU21(0x02,0x02);   //读取状态信息2
			SendASDU21(0xf4,0x05);
			return true;
		}*/
...
void	C103::SendASDU21(ST_BYTE readCode,ST_UINT16 GIN)
{
    ST_BYTE sendcode,Code,FCV;
	FCV=1;
	Code=3;
...
void	C103::ASDU10(ST_BYTE* Rebuf)
{
	...
	for (int i=0;i<yccount;i++)
	{
        ...
        {/*
            char msg[64];
            sprintf(msg,"fun: %d,inf:%d,i:%d",Rebuf[fIndex],Rebuf[fIndex+1]);
            ShowMessage(msg);*/
            char bvalue[4];
            bvalue[0]=vbegin[0];
            bvalue[1]=vbegin[1];
        ...
        {
/*            char msg[64];
            sprintf(msg,"fun: %d,inf:%d",Rebuf[fIndex],Rebuf[fIndex+1]);
            ShowMessage(msg);*/
            if(inf == 0x08)
            {
                int rtid = 1000;
                for(int k = 3;k>=0;k--){
                    for(int d = 0;d<8;d++){
                        ST_BYTE bvalue = (vbegin[k]>>d)&0x01;
                        this->UpdateValue(rtid++,bvalue);
                    }
                }
            }
            else if(inf = 0x09){
...
/*
        ...
        if(info && info->DataAreasCount > 1)
        {
            for(int j = 0;j<info->DataAreasCount;j++)
            {
                ST_UINT16 addrs = (ST_UINT16)info->DataAreas[j].addr;
                if(fun == (addrs / 256) )
                {
                    ShowMessage("get true areas!");
                    if(type == 0x07)
                    {
                        char bvalue[4];
                        bvalue[0]=vbegin[0];
                        ...
                        const ST_DataAreaItem& itemref = this->GetDevice()->GetDeviceInfo()->DataAreas[j].items[inf-1];
                        this->UpdateValue(itemref.id,fvalue);
                    }
                    else if(type == 0x03)
                    {
                        //ShowMessage("")
                        int rtid = this->GetDevice()->GetDeviceInfo()->DataAreas[j].items[0].id;
                        for(int k = 3;k>=0;k--)
                        {
                            for(int d = 0;d<8;d++){
                                ST_BYTE bvalue = (vbegin[k]>>d)&0x01;
...
void	C103::ASDU1(ST_BYTE* Rebuf)
{
    ...
    if(fun == 0xA0)
    {
        ...
        if(inf == 65)
        {
            ...
            for(int i = 0;i<30;i++)
            {
                this->UpdateValue(1000+i,0);
                this->UpdateValue(2000+i,0);
            }
            return ;
        }
                //ShowMessage("A0");
        char msg[64];
        sprintf(msg,"fun :%d,inf: %d",fun,inf);
        ShowMessage(msg);
        if(inf<=10)
            this->UpdateValue(inf-1,fvalue);
        else if(inf >19 )
            this->UpdateValue(inf-12,fvalue);
		...
		for (int i = 0; i < count; ++i)
		{
            ...
            if(fun == 0xA0)
            {
                ...
                if(inf<=10)
                    ...
                    this->UpdateValue(inf-10,fvalue);
...
/*    ST_BYTE sendcode,Code,FCV;
	...
	sendbuf[14]=0x24;
	sendbuf[15]=task.taskValue == 1 ? 0x01: 0x02;//0x04;  //GIN
