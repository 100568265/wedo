...
void	CModbusRTU::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
	...
	interval = 1000;//(interval > 2000 ? interval : 2000);
...
inline ST_UINT64 wswap64_res(ST_UINT64 value)
{
	//09 41 9D 20 84 BB 12 B2 => 41 09 20 9D BB 84 B2 12
	ST_UINT64 tempV = 0x0000000000000000,resValue = 0x0000000000000000;
	tempV = (bswap16(((value>>48)&0x000000000000FFFF)));
	resValue = (resValue|(tempV<<48));
	tempV = (bswap16(((value>>32)&0x000000000000FFFF)));
	resValue = (resValue|(tempV<<32));
	tempV = (bswap16(((value>>16)&0x000000000000FFFF)));
	resValue = (resValue|(tempV<<16));
	tempV = (bswap16(((value)&0x000000000000FFFF)));
	resValue = (resValue|tempV);
	return resValue;//(((ULONGLONG)wswap32(value & 0x00000000FFFFFFFF) ) | (wswap32(value & 0xFFFFFFFF00000000)));
}
...
void  CModbusRTU::ProcessMemoryDouble(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
	...
	else if (itemref.dataLen == 8)
	{
		switch (itemref.codeType) {
			...
			case 4: {
			    ST_UINT64 value = wswap64_res(convert::bytes_to<uint64_t>(buf + itemref.addr, itemref.dataLen));
				fvalue = (*((ST_DOUBLE*)&value));
			} break;
			case 5: {
