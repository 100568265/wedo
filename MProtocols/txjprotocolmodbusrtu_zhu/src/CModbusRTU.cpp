...
void	CModbusRTU::OnRead(ST_BYTE* pbuf,ST_INT& readed)
{
    ...
    ST_INT len = this->GetCurPort()->PickBytes(pbuf, 2, interval);
    if(len < 2)
    {
    ...
    len = this->GetCurPort()->PickBytes(pbuf, 2, interval);
    ...
    {
        ShowMessage ("No Function Code Found!");
        this->GetCurPort()->Clear();
        return;
    }
...
void CModbusRTU::ProcessMemory(ST_BYTE* buf, ST_BYTE count)
{
    ...
    for(ST_INT k = 0; k < itemsize; k++)
    {
        ...
        case VALType_SByte:
            ...
            break;
        case VALType_Int16:
            ProcessMemoryInt16    (buf, itemref);
            break;
        case VALType_Int32:
            ProcessMemoryInt32    (buf, itemref);
            break;
        case VALType_Byte :
            ProcessMemoryByte     (buf, itemref);
            break;
        case VALType_UInt16:
            ProcessMemoryUInt16   (buf, itemref);
            break;
        case VALType_UInt32:
            ProcessMemoryUInt32   (buf, itemref);
            break;
        case VALType_Float:
            ProcessMemorySingle   (buf, itemref);
            break;
        case VALType_Boolean:
            ProcessMemoryBoolean  (buf, itemref);
            break;
        case VALType_String:
            ProcessMemoryString   (buf, itemref);
            break;
        case VALType_Binary:
            ProcessMemoryBytes    (buf, itemref);
            break;
        case VALType_Double:
            ProcessMemoryDouble   (buf, itemref);
            break;
        case VALType_Decimal:
            ProcessMemoryDecimal  (buf, itemref);
            break;
        case VALType_DateTime:
            ProcessMemoryDateTime (buf, itemref);
            break;
        case VALType_Int64:
            ProcessMemoryInt64    (buf, itemref);
            break;
        case VALType_UInt64:
            ProcessMemoryUInt64   (buf, itemref);
            break;
        default:
            break;
        }
...
void  CModbusRTU::ProcessMemoryInt32(ST_BYTE* buf, const ST_DataAreaItem& itemref)
{
    ...
    if(itemref.dataLen == 1)
    {
        ...
        if((itemref.codeType == 2) || (itemref.codeType == 3))
        {
            ...
            fvalue = (ST_DOUBLE)(*((ST_CHAR*)&value));
    }
    else if(itemref.dataLen == 2)
    {
        if((itemref.codeType == 2))
